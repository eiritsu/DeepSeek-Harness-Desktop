...
/** Generic-file-disabled base for image-only tool fixtures. */
abstract class ImageOnlyAttachmentStore extends AttachmentStore {
  readonly fileLimits = { maxFileBytes: 1, maxFilesPerMessage: 1, maxMessageFileBytes: 1 }
  saveFile(): Promise<never> { return Promise.reject(new Error('read_image fixture does not save generic files')) }
  readFile(): Promise<never> { return Promise.reject(new Error('read_image fixture does not read generic files')) }
}
...
describe('argument and service preconditions', () => {
  ...
  it('refuses a media type the deployment does not accept', async () => {
    ...
    class JpegOnlyStore extends ImageOnlyAttachmentStore {
...
describe('image admission failures', () => {
  ...
  it('passes storage faults and non-attachment failures through unchanged', async () => {
    ...
    class FailingStore extends ImageOnlyAttachmentStore {
  ...
  it('omits the display name when the store returns a reference without one', async () => {
    ...
    class NamelessStore extends ImageOnlyAttachmentStore {
      readonly imageLimits: ImageAttachmentLimits = Object.freeze({
        maxImageBytes: 1024,
        maxImagesPerMessage: 1,
        maxMessageImageBytes: 1024,
  ...
  it('names the on-disk dimensions and coordinate multiplier when storage downscales', async () => {
    ...
    class DownscalingStore extends ImageOnlyAttachmentStore {
      readonly imageLimits: ImageAttachmentLimits = Object.freeze({
        maxImageBytes: 1024,
        maxImagesPerMessage: 1,
        maxMessageImageBytes: 1024,
