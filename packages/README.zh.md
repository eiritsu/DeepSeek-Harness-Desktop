...
| [`computer-use/`](computer-use/README.zh.md) | 按名称独占注册桌面提供方与嵌入式 Cua Driver 原生提供方 |
...
| [`experimental/`](experimental/README.zh.md) | 带有显式公开 npm 例外的实验性原型 |
...
大多数组属于产品组，提供稳定 API。例外：`e2b/` 是 POC，`experimental/` 仅发布显式允许列表中的包，`test-support/`、`runtime-diagnostics/` 与 `util/` 是兼容性预期较低的支持组。
