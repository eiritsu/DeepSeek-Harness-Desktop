...
/**
 * One positional replacement of an earlier model-visible surface range, used by
 * {@link Agent.retryInterrupted}. Endpoints are inclusive seqs of current surface
 * nodes in surface order; `sourceEventSeqs` must list every shadowed node.
 */
export interface SurfaceReplacement {
  /** Inclusive seq of the first shadowed surface node. */
  readonly startSeq: SessionSeq
  /** Inclusive seq of the last shadowed surface node. */
