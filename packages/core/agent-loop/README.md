...
`Agent.resendPrompt(message, replacement)` is the loop's one-shot surface-replacement input: it stores the replacement keyed by the message id and wakes an ordinary follow-up turn, and the first attempt appends that message with the replacement's `surfaceOp` and complete `sourceEventSeqs` instead of an append. The new turn runs the ordinary pre-step, current request configuration, and streaming path. `Agent.resumeTurn()` instead spends one model request over the current surface without admitting a message, so a cancelled Turn continues from committed history as it stands; its first step runs even when pre-step claims nothing, and the pending resume is consumed by exactly one Turn. The pending replacement never overwrites a live one and is cleared when a rejected pre-step, a rewritten or emptied admitted batch, an admission throw, cancellation, or driver teardown leaves it unconsumed.
