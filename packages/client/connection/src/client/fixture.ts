...
import type {
  AttachmentIdType, AttachmentRef, FileAttachmentRef, ImageAttachmentRef,
...
/** Return the decoded byte count of one canonical Base64 payload. */
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const attachments = new Map<string, { attachment: AttachmentRef; data: string }>([[
      ...
      delete: (request) => {
        ...
        for (;;) {
          ...
          for (const summary of sessions) {
            if (summary.parentSessionId !== undefined && targets.has(summary.parentSessionId)) {
              targets.add(summary.sessionId)
            }
          }
          if (before === targets.size) break
        }
        const childSessionIds = [...targets].filter(id => id !== sessionId)
        if (childSessionIds.length > 0 && !recursive) {
          return err(request, {
            code: 'session-has-children',
            message: `session ${sessionId} has descendants`,
            details: { sessionId, childSessionIds },
          })
        }
        const deletedSessionIds = [...targets]
        for (const id of deletedSessionIds) {
          const index = sessions.findIndex(summary => summary.sessionId === id)
          ...
          const archivedIndex = archivedSessionIds.indexOf(id)
      ...
      prompt: (request) => {
        ...
        const durable: ContentBlock[] = content.map((block) => {
          ...
          const attachment: ImageAttachmentRef = {
            ...
            bytes: Math.max(1, base64ByteLength(block.data)),
            width: 160,
            height: 90,
            ...block.name === undefined ? {} : { name: block.name },
          }
