...
describe('createFixtureApi', () => {
  ...
  it('mux open replays subscribed sessions and resident interactions with stable rpcIds', async () => {
    ...
    expect(first[11]?.payload).toMatchObject({
      type: 'session/projection', sessionId: 'fx-alpha', key: 'fileLimits',
      value: { maxFilesPerMessage: 20, maxFileBytes: 32 * 1024 * 1024 },
    })
  ...
  it('records the exact decoded byte count for generic file prompt blocks', async () => {
    const api = createFixtureApi()
    const created = await api.sessions.create(req({}))
    if (!created.result.ok) throw new Error('create failed')
    ...
    const prompted = await api.sessions.prompt(req({
      sessionId,
      mode: 'queue' as const,
      content: [{ type: 'file' as const, mediaType: 'text/plain', data: 'eA==', name: 'notes.txt' }],
    }))
    if (!prompted.result.ok) throw new Error('prompt failed')
    const history = await api.sessions.history(req({ sessionId }))
    if (!history.result.ok) throw new Error('history failed')
    const message = history.result.value.events.find(entry => entry.event.type === 'user/message')
    const content = message?.event.type === 'user/message' ? message.event.data.content : undefined
    expect(content).toHaveLength(1)
    const block = content?.[0]
    if (block?.type !== 'file') throw new Error('file block missing')
    expect(block.attachment).toMatchObject({ mediaType: 'text/plain', bytes: 1, name: 'notes.txt' })
  })
