...
export const zh = {
  ...
  dataNav: '桌面数据',
  dataTitle: '备份本地数据',
  dataIntro: '配置备份与会话数据库分开处理；导入和清空会安全停止并重新启动后端。',
  dataUnavailable: '当前页面不在受支持的桌面壳中。',
  configBackupTitle: '备份本地配置',
  configBackupIntro: '导出脱敏配置，或从备份恢复。密钥、会话正文和附件不会导出。',
  sessionBackupTitle: '备份会话数据库',
  sessionBackupIntro: '导出或恢复权威 SQLite Session 数据库；文件中包含会话正文和持久化会话元数据。',
  resetAllTitle: '清空全部本地数据',
  resetAllIntro: '删除会话、凭据、工作区、插件状态和 Skill 数据；应用本体不会删除。',
  exportData: '导出',
  importData: '导入',
  resetData: '清空数据',
  resetSessions: '清空会话',
...
export const en: Record<DeepseekFilesLocaleKey, string> = {
  ...
  dataNav: 'Desktop data',
  dataTitle: 'Back up local data',
  dataIntro: 'Configuration backups and the Session database are handled separately. Import and reset safely stop and restart the backend.',
  dataUnavailable: 'This page is not running in a supported desktop shell.',
  configBackupTitle: 'Back up local configuration',
  configBackupIntro: 'Export sanitized configuration or restore it from a backup. Secrets, Session transcripts, and attachments are excluded.',
  sessionBackupTitle: 'Back up the Session database',
  sessionBackupIntro: 'Export or restore the authoritative SQLite Session database. The file contains transcripts and persisted Session metadata.',
  resetAllTitle: 'Clear all local data',
  resetAllIntro: 'Delete Sessions, credentials, Workspaces, plugin state, and Skills without removing the application.',
  exportData: 'Export',
  importData: 'Import',
  resetData: 'Clear data',
  resetSessions: 'Clear Sessions',
  dataWorking: 'Working…',
  dataDone: 'Operation completed.',
  dataFailed: 'Operation failed. Check the desktop log.',
}
