...
- **不能任意输入推理强度**——composer 仅提供 `Default`、`Off`、`Low`、`High`、`Max`；提供方可以拒绝自己未实现的显式档位。
