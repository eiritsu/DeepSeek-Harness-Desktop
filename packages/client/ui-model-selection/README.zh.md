...
Web GUI 允许用户通过 `/model` 弹窗或 composer 模型控件切换既有会话使用的模型与推理（reasoning）强度。两个界面呈现同一组按提供方分组的选择，且 composer 的推理菜单始终提供同样的七个等级：Default、Minimal、Low、Medium、High、XHigh、Max。完整选择从下一次请求开始生效；运行中的步骤保留其启动时的模型与推理强度。如果没有适配器可以服务会话路由，composer 会保持停用，直至路由恢复可用。
...
模型按提供方分组。composer 菜单显示模型名称与一份固定推理强度列表——Default、Minimal、Low、Medium、High、XHigh、Max，顺序如上——适用于每个模型，包括没有推理元数据的模型；适配器公布的等级、名称与声明的默认值绝不改变该列表或所作选择，Default 表示选择不携带显式推理强度。`/model` 弹窗显示提供方名称与目录说明；其中两个内置 DeepSeek 模型的说明使用当前语言，外部提供方说明保持原文。在该弹窗选择模型只提交路由，因此模型切换绝不把目录元数据的默认值写入选择。选择 Host 不接受的推理强度会通过 composer 的失败 toast 报告。
...
- **固定的推理强度词汇表**——composer 始终提供 Default、Minimal、Low、Medium、High、XHigh、Max 七个等级，与适配器公布的元数据无关；适配器拒绝某一等级时通过失败 toast 报告，且不存在任意推理强度输入。
