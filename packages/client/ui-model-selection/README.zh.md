...
模型按提供方分组。菜单只显示模型与推理强度名称；目录中的说明仍可供其他消费方使用。`/model` 弹窗应用所选模型的默认推理强度；composer 随后可以选择任一已公布的推理强度。适配器没有确切目录元数据时提供共享的 `Off`、`Low`、`High`、`Max` 回退；不存在任意推理强度输入。
...
- **不能任意输入推理强度**——composer 仅提供确切模型由适配器公布的推理强度，其中包括未列出模型使用的适配器共享回退。
