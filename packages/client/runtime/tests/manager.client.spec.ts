...
describe('list lifecycle', () => {
  ...
  it('removes every Host-confirmed descendant after a permanent delete', async () => {
    const api = new FakeApiClient()
    api.onList = () => Promise.resolve(ok({ items: [summary(S1), summary(S2)] as never[] }))
    api.onDelete = payload => Promise.resolve(ok({ deletedSessionIds: [S2, payload.sessionId] }))
    const manager = new SessionManager(api, fakeRemote())
    await manager.refreshList()
    const resident = manager.get(S2)

    const result = await manager.delete(S1)

    expect(result).toEqual({ ok: true, value: { deletedSessionIds: [S2, S1] } })
