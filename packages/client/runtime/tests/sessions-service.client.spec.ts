...
describe('delete', () => {
  it('projects confirmed deletion and surfaces Host rejection', async () => {
    const b = bench()
    await feedList(b, [{ id: 'root' }, { id: 'child', parentId: 'root' }])
    b.api.onDelete = () => Promise.resolve(ok({ deletedSessionIds: [sid('child'), sid('root')] }))

    await expect(b.svc.delete(sid('root'))).resolves.toEqual(['child', 'root'])
    expect(b.svc.list.getSnapshot().ids).toEqual([])

    b.api.onDelete = () => Promise.resolve(err({
      code: 'agent-busy', message: 'still running', details: { reason: 'running' },
    }))
    await expect(b.svc.delete(sid('root'))).rejects.toThrow('still running')
  })
})
