...
export class SessionManager {
  ...
  /**
   * Permanently delete one Session tree and remove every confirmed id locally.
   * @param sessionId - root Session identity to delete.
   * @param recursive - whether to include every descendant Session.
   * @returns the Host result carrying the identities confirmed deleted.
   */
  async delete(
    sessionId: SessionId,
    recursive = true,
    ...
    try {
      ...
      if (result.ok) {
        for (const deletedId of result.value.deletedSessionIds) {
          this.recordMutation({ kind: 'remove', sessionId: deletedId })
          this.sessions.get(deletedId)?.handleRemoved()
          this.pendingBuffers.delete(deletedId)
          this.pendingInteractions.delete(deletedId)
        }
      }
      return result
    } catch (error: unknown) {
