...
import type { AttachmentIdType, AttachmentRef } from '@deepseek-ai/dsh-attachment'
...
export class Session implements SessionFace {
  ...
  async prompt(
    ...
    try {
      if (this.address === undefined) {
        ...
        const unsupportedAttachment = content.find(part => part.type !== 'text')
        if (unsupportedAttachment !== undefined) {
          const image = unsupportedAttachment.type === 'image'
          result = {
            ok: false,
            error: {
              ...
              details: { reason: image ? 'SUBAGENT_IMAGE_UNSUPPORTED' : 'SUBAGENT_FILE_UNSUPPORTED' },
            },
          }
        } else {
  ...
  ): Promise<RpcResult<{ attachment: AttachmentRef; data: Uint8Array }>> {
    try {
