 ...
 * except workspace Rename/Delete and session Rename/Fork/Archive/Attach/Delete; the session
 ...
 * @param props.onAttach - open Workspace selection for a session.
 ...
 */
export function SessionNodeItem({ node, currentId, now, onOpen, onRename, onFork, onArchive, onAttach, onDelete, drag, flat = false, t }: {
  node: SessionNode
  currentId: string | undefined
  now: number
  ...
  /** Open Workspace selection for this session. */
  onAttach?: (id: SessionNode['id'], currentTitle: string) => void
  /** Permanently delete this session (browser-owned confirmation). */
  ...
  const sessionMenuItems = [
    ...
    ...(onAttach === undefined ? [] : [{ id: 'attach', label: t('menu.attachSession'), icon: <IconFolderClose16 /> }]),
        ...
        <span className={css.rowActions}>
          <Menu
            ...
            onSelect={(id) => {
              ...
              if (id === 'attach') onAttach?.(node.id, title)
