...
describe('WorkspaceBrowser', () => {
  ...
  it('confirms permanent Session deletion and closes only after it succeeds', async () => {
    ...
    expect(screen.getByRole('dialog', { name: '删除会话' }).textContent)
      .toContain('将永久删除“alpha-s”的会话记录及其所有分叉或子会话')
  ...
  it('keeps the Session delete dialog open after failure and allows cancellation', async () => {
    const deleteSession = vi.fn(async () => { throw new Error('delete exploded') })
    mount({
      useSessions: hook(sessionState([summary('alpha-s', 1)])),
      useWorkspaces: hook(workspaceState([workspace('alpha', ['alpha-s'])])),
      deleteSession,
    ...
    expect((await screen.findByRole('alert')).textContent).toBe('delete exploded')
