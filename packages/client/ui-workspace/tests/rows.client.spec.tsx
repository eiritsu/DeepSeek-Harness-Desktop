...
describe('workspace browser rows', () => {
  ...
  it('blank New Session rows expose identity, archive, and delete without content verbs or time', () => {
    vi.useFakeTimers()
    try {
      const node: SessionNode = {
      ...
      render(<SessionNodeItem node={node} currentId={node.id} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} t={t} />)
      const actions = screen.getByRole('button', { name: /会话.*的操作/ })
  ...
  it('session row menu dispatches rename, fork, copy, archive, and permanent delete', async () => {
    const onOpen = vi.fn()
    const onRename = vi.fn()
    const onFork = vi.fn()
    ...
    fireEvent.click(screen.getByRole('button', { name: '会话“One”的操作' }))
    await act(async () => { fireEvent.click(screen.getByRole('menuitem', { name: '复制 Session ID' })) })
