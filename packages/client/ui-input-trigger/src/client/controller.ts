...
export class InputTriggerController {
  ...
  /**
   * Toggle the Add launcher over every slash source in display order.
   * @param hit - synthetic trigger hit anchoring the launcher to the current draft.
   */
  toggleLauncher(hit: TriggerHit): void {
    if (this.disposed) return
    if (this.launcher.getSnapshot() === '*' && this.menu.getSnapshot().open) {
      this.dismiss()
      return
    }
