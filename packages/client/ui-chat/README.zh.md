...
使用本包可在浏览器中渲染已记录的 Session 对话，包括历史图片、本地化操作和滚动位置恢复。识别出的附件文本保留在默认收起项中以供检查。已完成轮次的过程行与打包的历史 Assistant 连续消息保持收起，同时最终答案继续可见。本地 transcript（文本记录）与 steering（中途引导）会立即显示，直到权威记录替换它们；排队中的提交不进入 Chat。文件提及提供方同时接收当前查看的 Session ID 与收尾轮次属主，使继承历史中的链接可以指向 fork。本包不组装模型请求。
...
Session 级 token pill 读取持久 `tokenUsage` 投影。已知的正数缓存读取统一标为“缓存命中”：pill 显示它占全部计费输入的保守比例，弹层在百分比旁显示精确读取量。省略缓存读取统计的调用仍计入未缓存输入，因此缺失统计不会抬高该比例；没有任何调用报告正数读取时显示“缓存未报告”，而不是声称“缓存命中 0%”。
