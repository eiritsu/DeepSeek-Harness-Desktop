...
describe('attachment rejection copy', () => {
  ...
  it('maps user-solvable reasons to limit-naming copy', () => {
    ...
    expect(attachmentErrorText(t, 'TOO_MANY_FILES', limits, fileLimits)).toBe('一条消息最多添加 20 个文件')
    expect(attachmentErrorText(t, 'FILE_TOO_LARGE', limits, fileLimits)).toBe('单个文件不能超过 32MB')
    expect(attachmentErrorText(t, 'FILES_TOO_LARGE', limits, fileLimits)).toBe('文件总大小超过 100MB，请移除部分文件')
    expect(attachmentErrorText(t, 'INVALID_FILE_MEDIA_TYPE', limits, fileLimits)).toBe('文件信息无效，请重新添加后再试')
  })

  it('folds unknown reasons and limit reasons without projected limits into the send-failed line', () => {
    expect(attachmentErrorText(t, 'INVALID_IMAGE_BASE64')).toBe('附件发送失败（INVALID_IMAGE_BASE64），请重新添加后再试')
    expect(attachmentErrorText(t, 'TOO_MANY_IMAGES')).toBe('附件发送失败（TOO_MANY_IMAGES），请重新添加后再试')
    expect(attachmentErrorText(t, 'IMAGE_TOO_LARGE')).toBe('附件发送失败（IMAGE_TOO_LARGE），请重新添加后再试')
    expect(attachmentErrorText(t, 'IMAGES_TOO_LARGE')).toBe('附件发送失败（IMAGES_TOO_LARGE），请重新添加后再试')
    expect(attachmentErrorText(t, 'IMAGE_DIMENSION_TOO_LARGE')).toBe('附件发送失败（IMAGE_DIMENSION_TOO_LARGE），请重新添加后再试')
  })
})
