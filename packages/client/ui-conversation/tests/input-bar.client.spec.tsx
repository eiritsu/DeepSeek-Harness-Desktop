...
interface BenchOptions {
  ...
  /** The `fileLimits` projection value for generic-file pre-checks. */
  fileLimits?: {
    maxFileBytes: number
...
describe('image draft rail', () => {
  ...
  it('accepts non-image formats through the generic file path', () => {
    const addImages = vi.fn(() => null)
    const result = bench({
      addImages,
      imageLimits: {
  ...
  it('pre-checks generic file limits independently from image limits', () => {
    const addImages = vi.fn(() => null)
    const result = bench({
      addImages,
      fileLimits: { maxFileBytes: 8, maxFilesPerMessage: 1, maxMessageFileBytes: 8 },
    })
    const files = [
      new File([new ArrayBuffer(4)], 'a.pdf', { type: 'application/pdf' }),
      new File([new ArrayBuffer(4)], 'b.bin'),
    ]
    act(() => { attachmentOwner(result.slotCalls).onAddImages(files) })
