...
import type { AttachmentRef, ImageMediaType } from '@deepseek-ai/dsh-attachment'
...
/** Browser fallback matching the generic-file wire contract. */
const UNKNOWN_FILE_MEDIA_TYPE = 'application/octet-stream'
...
export class ConversationController extends Service implements IConversation {
  ...
  async serializeDraftImages(imageIds: readonly DraftAttachmentId[]): Promise<readonly SubmitImageAttachment[]> {
    const attachments = this.draftImages(imageIds)
    if (attachments.length !== imageIds.length) {
      throw new Error('conversation.serializeDraftImages: one or more draft attachments are no longer available')
    }
    if (attachments.some(attachment => attachment.kind !== 'image')) {
      throw new Error('Commands do not accept file attachments; remove the files before running the command.')
    }
   ...
   */
  resolveImage(sessionId: SessionId, attachment: AttachmentRef): Promise<string> {
  ...
  private serializeAttachments(attachments: readonly ComposerAttachment[]): Promise<Parameters<SessionFace['prompt']>[0]> {
    return Promise.all(attachments.map(async (attachment) => {
      ...
      return {
        type: 'file' as const,
        mediaType: file.type || UNKNOWN_FILE_MEDIA_TYPE,
...
/**
 * Identify browser files that use the normalized raster-image path.
 * @param value - browser-declared media type.
 * @returns whether the media type is an accepted raster image type.
 */
