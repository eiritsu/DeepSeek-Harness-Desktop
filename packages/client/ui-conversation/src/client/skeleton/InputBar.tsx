...
export function InputBar({
  ...
  const intakeImages = useCallback((files: readonly File[]): void => {
    ...
    const rejected = ((): string | null => {
      const incomingImages = files.filter(file => isImageMediaType(file.type))
      const incomingFiles = files.filter(file => !isImageMediaType(file.type))
      const heldImages = attachments.filter(attachment => attachment.kind === 'image')
      const heldFiles = attachments.filter(attachment => attachment.kind === 'file')
      if (imageLimits !== undefined && incomingImages.length > 0) {
        if (incomingImages.some(file => !(imageLimits.mediaTypes as readonly string[]).includes(file.type))) {
          return t('image.unsupportedType')
        }
        if (heldImages.length + incomingImages.length > imageLimits.maxImagesPerMessage) {
      ...
      if (fileLimits !== undefined && incomingFiles.length > 0) {
        if (heldFiles.length + incomingFiles.length > fileLimits.maxFilesPerMessage) {
        ...
        const total = heldFiles.reduce((sum, attachment) => sum + attachment.file.size, 0)
