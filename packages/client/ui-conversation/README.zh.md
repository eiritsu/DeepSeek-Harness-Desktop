...
可见 Assistant 增量使用累计的 animation-frame publication lane，在下一次绘制机会物化；终态 settlement 与结构性变更立即发布。

<a id="shell-and-standard-props"></a>
## Shell 与标准 props

输入框分别注册「文件」与「文件夹」命令动作，负责它们的标题、可用性和原生选择器回调。「文件」接受浏览器支持的全部文件类型；「文件夹」要求浏览器或嵌入 shell 返回递归目录内容，并把返回的每个文件送入同一通用文件上传队列。菜单可用性与实际调用都读取已挂载输入框当前的附件接收策略。输入框卸载或锁定后两个动作都不可用，插件 dispose（资源释放）时移除注册。回调绑定留在输入模块内部。
