...
describe('ui-agent-preset apply', () => {
  ...
  it('adopts the persisted default when a blank new session mounts', async () => {
    const { ctx, slots, calls, moveDefault } = await bench()
    moveDefault()
    declareRoot(slots)
    const conversation = declareConversation(slots)
    ctx.provide('conversation', {} as never)
  ...
  it('recomposes the current blank session when the default changes', async () => {
    const { ctx, slots, calls, moveDefault } = await bench()
    declareRoot(slots)
    const conversation = declareConversation(slots)
    ctx.provide('conversation', {} as never)
  ...
  it('does not recompose a started session when the default changes', async () => {
    const { ctx, slots, calls, moveDefault } = await bench()
    declareRoot(slots)
    const conversation = declareConversation(slots)
    ctx.provide('conversation', {} as never)
    ...
    ctx.remote.$dispatch('settings/document-updated', ['agent-presets', 1])
    await vi.waitFor(() => {
      expect(seat.hooks.agentPresetSeat.getSnapshot().options).toHaveLength(2)
    })
  ...
  it('does not recompose a blank session already using the changed default', async () => {
    const { ctx, slots, calls, moveDefault } = await bench()
    moveDefault()
    declareRoot(slots)
    const conversation = declareConversation(slots)
