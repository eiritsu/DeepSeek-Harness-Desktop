...
A session's preset becomes fixed when its first turn starts, because later replacement would leave history produced under another tool set. Changing the default therefore recomposes the current blank new session immediately and governs sessions opened afterwards, while a session that has started keeps its composition.
...
The row re-reads on `settings/changed` for its own namespace and on `connection/reset`: the roster is a live directory and the default is a settings field, so an external edit or a reconnect can both move it. The new-session chip adopts that default both when it mounts and when the setting changes; when its current session is still blank, it commits the choice through `agentPreset.select`. An explicit choice staged on the chip takes precedence.
