...
export function apply(ctx: ClientContext): void {
  ...
  ctx.inject(['slots', 'conversation', 'sessions', 'workspaces'], (scope: ClientContext) => {
    ...
    const seatInjected = (): AgentPresetSeatInjected => ({
      ...
      // A host restart can restore a blank session before the chip mounts;
      // reconcile it with the persisted default even though no settings
      // change event occurs in this browser process.
      load: () => seat.mount(),
    ...
    scope.effect(() => {
      ...
      // A settings change governs the current blank session represented by
      // this screen, not only a session created after the next reload. The
      // controller preserves an explicit staged pick and never recomposes a
      // session that has started.
      const settingsMoved = scope.remote.$on('settings/document-updated', (ns) => {
        if (ns !== AGENT_PRESET_SETTINGS_NS) return
        void seat.adoptDefault()
      })
