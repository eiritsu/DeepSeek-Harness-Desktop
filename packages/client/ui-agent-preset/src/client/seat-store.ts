...
export class AgentPresetSeatController {
  ...
  /** The latest switch this controller committed, for a late mount read. */
  private applied: { sessionId: string; agentPreset: string } | undefined
  ...
  private async adoptLoadedDefault(): Promise<void> {
    const session = this.currentSession()
    if (this.staged !== undefined || this.fallback === '' || session === undefined || !session.blank) return
    if (session.agentPreset === this.fallback) return
    this.stage(this.fallback)
    await this.apply()
  }

  /**
   * Load the chip at mount and reconcile a restored blank session with the
   * persisted default. A switch this controller just committed wins when the
   * roster read lands after that switch.
   * @returns once the roster and any permitted blank-session switch settle.
   */
  async mount(): Promise<void> {
    await this.load()
    const session = this.currentSession()
    if (session !== undefined
      && this.applied?.sessionId === session.id
      && this.applied.agentPreset === session.agentPreset) return
    await this.adoptLoadedDefault()
  }

  /**
   * Adopt the deployment default after its setting changes.
   *
   * A current blank session is still the new session represented by this
   * chip, so it is recomposed immediately. A started session keeps its own
   * composition, and an explicit staged pick wins over the global setting.
   * @returns once the roster and any permitted blank-session switch settle.
   */
  async adoptDefault(): Promise<void> {
    await this.load()
    await this.adoptLoadedDefault()
  }
  ...
  async apply(): Promise<void> {
    ...
    try {
      ...
      this.applied = { sessionId: session.id, agentPreset: response.result.value.agentPreset }
