...
文件拖到页面上方时，全视口遮罩显示拖放提示，包括插画和标题；接受拖放时还会显示一行限制说明。浏览器文件拖放进入普通附件接收链路。Swift shell 的 `dsh:native-drop` bridge 把受限制的文件字节转为浏览器文件；文件夹与超过 bridge 限制的文件会变成带引号的 `@` 路径引用。两条路径都遵守持有方的实时接收锁定状态。
