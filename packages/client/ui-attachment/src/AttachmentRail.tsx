...
export interface AttachmentRailItem {
  ...
  removeLabel: string
  /** Display name used by a generic file card. */
  fileName?: string
  /** Short file type or size line. */
  fileMeta?: string
}
        ...
        {items.map(item => (
          <div key={item.id} className={css.item}>
            {item.previewUrl === undefined
              ? (
                <div className={css.fileCard} title={item.fileName}>
                  <span className={css.fileGlyph} aria-hidden>
                    <svg viewBox="0 0 24 24">
                      <path d="M7 3.75h6.2L17 7.55v12.7H7z" />
                      <path d="M13 3.75v4h4M9.5 12h5M9.5 15h5" />
                    </svg>
                  </span>
                  <span className={css.fileText}>
                    <span className={css.fileName}>{item.fileName}</span>
                    <span className={css.fileMeta}>{item.fileMeta}</span>
                  </span>
                </div>
              )
              : (
