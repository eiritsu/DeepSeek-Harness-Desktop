...
- Every package here uses the `@deepseek-ai/dsh-experimental-*` npm prefix. Packages are private by default and omit `publishConfig`; the allowlisted Agent Teams packages and the Cua Driver MCP provider are explicit public exceptions that keep their experimental names, set `publishConfig.access: public`, and join the dsh release family.
