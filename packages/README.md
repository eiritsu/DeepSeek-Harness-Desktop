...
| [`computer-use/`](computer-use/README.md) | Exclusive named desktop-provider registration and the embedded Cua Driver native provider |
...
Most groups are product — stable API. The exceptions: `e2b/` is a POC, `experimental/` publishes only explicitly allowlisted packages, and `test-support/`, `runtime-diagnostics/`, and `util/` are support with lower compatibility expectations.
