...
Retrying an interrupted answer is its own endpoint. `session.retryInterrupted` requires an idle Agent and resolves the target from the Session's current surface: the last surface node must be the addressed interrupted `assistant/message`, the latest closed Turn must own it, and the Turn's replayable prompt — an ordinary user message or a previous retry's `assistant-retry` — must be reachable with only injected user-role context and system nodes between them. On acceptance the Host admits a replacement `user/message` carrying the durable prompt content and the merge-extensible `assistant-retry` source, so the next Turn replays the prompt once and shadows the interrupted partial while the append-only log keeps it for audit. A tool call or result, a second assistant answer, a second replayable prompt, a running Agent, or no replayable prompt rejects with `session/retry-unavailable`; one Session accepts a concurrent retry once.
