...
Permanent delete preflights the complete descendant tree before mutation. It refuses if any target is running or still owns persistence, disposes retained idle Agents, deletes children before parents, and detaches every deleted id from all Workspaces. A non-recursive Host request refuses a Session with children; the current Client confirmation deliberately requests recursive deletion.

Editing and re-sending the latest Turn's prompt is its own endpoint. `session.resend` requires an idle Agent and addresses the durable id of the latest closed Turn's ordinary opening `user/message`. That message must be a current surface node and the Turn's sole replayable prompt, so an injected-context Turn, a shadowed prompt, a steering message, a historical or open Turn, and a Turn with more than one replayable prompt reject. The replacement spans from the prompt through the Turn's last surface node and cites those shadowed nodes plus the Turn's later log-only `tool/call` and `assistant/attempt` events, so assistant answers, tool results, and attempts stop re-entering the derived history of the re-sent request. Omitting `content` replays the durable prompt verbatim, which is exactly a retry; supplied text-only content replaces the prompt text while the durable attachment references stay in place and their recognition text regenerates. On acceptance the Host admits a replacement `user/message` carrying the merge-extensible `assistant-retry` source whose `retryOf` names the shadowed user message, so the next Turn replays the prompt once and shadows the previous answer while the append-only log keeps it for audit. `retryOf` reads the current `user-message` address, the assistant settlement address, and the bare interrupted message id written by released writers, while current writers always write the user-message address. A running Agent or an unaddressable message rejects with `session/resend-unavailable`; one Session accepts a concurrent resend once.

Resuming the latest stopped Turn is a separate endpoint. `session.resume` requires an idle Agent whose latest Turn ended `aborted` or `interrupted` after exactly one replayable prompt. It spends one model request over the current surface without admitting a user message, so the stopped Turn's committed tool results stay in model history and no earlier request is replayed; a running Agent, a finished Turn, or a Turn without exactly one replayable prompt rejects with `session/resume-unavailable`, and one Session accepts a concurrent resume once.
