...
Retrying the latest safe answer is its own endpoint. `session.retryInterrupted` requires an idle Agent and resolves the target from the Session's current surface and durable settlements. Its address is a typed union: a surface `assistant/message` id — interrupted, or an ordinary answer whose Turn ended `completed` — or the `seq` of a log-only `assistant/attempt` whose Turn ended `aborted`/`interrupted`. For a message address the last surface node must be that message; for an attempt address the latest closed Turn must end `aborted`/`interrupted`, carry that attempt as its only settlement, and hold no `assistant/message`. In both cases the Turn's replayable prompt — an ordinary user message or a previous retry's `assistant-retry` — must be reachable with only injected user-role context and system nodes between them. On acceptance the Host admits a replacement `user/message` carrying the durable prompt content and the merge-extensible `assistant-retry` source whose `retryOf` holds the same durable address, so the next Turn replays the prompt once and shadows the previous answer while the append-only log keeps it for audit. `retryOf` reads both the current address and the bare interrupted message id written by released 0.1.20, while current writers always write the address. A tool call or result, a second assistant answer or attempt, a second replayable prompt, a running Agent, a message whose Turn did not complete, or no replayable prompt rejects with `session/retry-unavailable`; one Session accepts a concurrent retry once.
