...
重试最新安全回答是独立端点。`session.retryInterrupted` 要求 Agent 处于空闲，并从 Session 当前 surface 与持久 settlement 解析目标。其地址是带标签的联合类型：surface `assistant/message` 的 id——被中断，或其 Turn 以 `completed` 结束的普通回答——或一个仅写入日志的 `assistant/attempt` 的 `seq`，且其 Turn 以 `aborted`/`interrupted` 结束。使用 message 地址时，最后一个 surface 节点必须是该 message；使用 attempt 地址时，最新已结束 Turn 必须以 `aborted`/`interrupted` 结束、以该 attempt 作为唯一 settlement，且表面不含 `assistant/message`。两种情况都要求该 Turn 的可重放提示词——普通 user 消息或上一次 retry 的 `assistant-retry`——可达，其间只允许注入式 user 角色上下文与 system 节点。接受后，Host 接纳一条替换用 `user/message`，携带持久提示词内容与可合并扩展的 `assistant-retry` source，其 `retryOf` 持有同一持久地址；下一个 Turn 因此只重放一次提示词并遮蔽先前的回答，而 append-only 日志仍保留它以供审计。`retryOf` 同时读取当前地址与 released 0.1.20 写入的裸中断 message id，而当前写入方始终写入地址。出现 tool call 或 tool result、第二个 assistant 回答或 attempt、第二个可重放提示词、Agent 正在运行、Turn 未以 completed 结束的 message，或找不到可重放提示词，都会以 `session/retry-unavailable` 拒绝；同一 Session 的并发重试只接纳一次。
