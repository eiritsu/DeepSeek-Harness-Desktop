...
重试被中断的回答是独立端点。`session.retryInterrupted` 要求 Agent 处于空闲，并从 Session 当前 surface 解析目标：最后一个 surface 节点必须是指定的 interrupted `assistant/message`，最新已结束 Turn 必须拥有它，且该 Turn 的可重放提示词——普通 user 消息或上一次 retry 的 `assistant-retry`——必须可达，其间只允许注入式 user 角色上下文与 system 节点。接受后，Host 接纳一条替换用 `user/message`，携带持久提示词内容与可合并扩展的 `assistant-retry` source；下一个 Turn 因此只重放一次提示词并遮蔽被中断的半截回答，而 append-only 日志仍保留它以供审计。出现 tool call 或 tool result、第二个 assistant 回答、第二个可重放提示词、Agent 正在运行，或找不到可重放提示词，都会以 `session/retry-unavailable` 拒绝；同一 Session 的并发重试只接纳一次。
