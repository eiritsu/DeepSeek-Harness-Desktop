...
永久删除会在变更前预检完整后代树。任一目标仍在运行或仍占有 persistence 时都会拒绝；其余 retained idle Agent 会先 dispose，然后按子代优先顺序删除，并从全部 Workspace 脱离。非递归 Host 请求在 Session 有子代时拒绝；当前 Client 确认操作会显式请求递归删除。

编辑并重发最新轮次的提示词是独立端点。`session.resend` 要求 Agent 处于空闲，并以最新已结束轮次的普通开轮 `user/message` 的持久 id 寻址。该消息必须是当前 surface 节点，且是该轮次唯一可重放的提示词；因此注入式上下文轮次、已被遮蔽的提示词、steering 消息、历史或未结束轮次，以及含多个可重放提示词的轮次都会被拒绝。替换范围从该提示词起、覆盖到该轮次最后一个 surface 节点，并引用这些被遮蔽的节点以及该轮次其后的仅日志 `tool/call` 与 `assistant/attempt` 事件，使 assistant 回答、tool result 与 attempt 都不再进入重发请求的派生历史。省略 `content` 会原样重放持久提示词，这正是重试；传入仅文本内容则替换提示词文本，同时保留持久 attachment 引用并重新生成其识别文本。接受后，Host 接纳一条替换用 `user/message`，携带可合并扩展的 `assistant-retry` source，其 `retryOf` 指向被遮蔽的 user 消息；下一个 Turn 因此只重放一次提示词并遮蔽先前的回答，而 append-only 日志仍保留它以供审计。`retryOf` 同时读取当前的 `user-message` 地址、assistant settlement 地址，以及已发布写入方写入的裸中断 message id，而当前写入方始终写入 user-message 地址。Agent 正在运行或消息不可寻址时，以 `session/resend-unavailable` 拒绝；同一 Session 的并发重发只接纳一次。

继续最新已停止轮次是另一个独立端点。`session.resume` 要求 Agent 处于空闲，且其最新轮次在恰好一个可重放提示词之后以 `aborted` 或 `interrupted` 结束。它在该轮次当前 surface 上花费一次模型请求，而不接纳 user 消息，因此已停止轮次已提交的 tool result 保留在模型历史中，且不会重放更早的请求；Agent 正在运行、轮次已正常结束，或轮次不含恰好一个可重放提示词时，以 `session/resume-unavailable` 拒绝，且同一 Session 的并发继续只接纳一次。
