...
export function runPersistenceContract(name: string, make: () => Promise<ContractBackend>): void {
  describe(`SessionPersistence contract: ${name}`, () => {
    ...
    it('permanently deletes a materialized session and rejects an unknown identity', async () => {
      const { persistence, dispose } = await make()
      try {
        const id = SessionId('delete-materialized')
        ...
        await expect(persistence.delete(id)).rejects.toThrow(`session "${id}" not found`)
      } finally {
        await dispose()
      }
    })

    it('cancels an unmaterialized create intent and allows the identity to be reused', async () => {
      const { persistence, dispose } = await make()
      try {
        const id = SessionId('delete-lazy-intent')
        await persistence.create(meta(id))
        await persistence.delete(id)
        await persistence.create(meta(id))
        await persistence.append(id, oneTurnLog())
