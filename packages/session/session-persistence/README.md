...
| `delete(id): Promise<void>` | Permanently remove one cold Session identity. Live Sessions reject; an unmaterialized lazy create intent is cancelled without creating an artifact; unknown ids reject. Success emits `session-persistence/deleted` after the backend and coordinator state no longer expose the Session. |
...
| `deleteStored(id)` | Permanently remove one materialized Session and report whether it existed. The coordinator serializes this operation with prior per-id work and owns the public deleted event. |
...
- **No retention policy or trash** — `delete` is immediate and permanent; scheduled pruning and recoverable deletion remain upper-layer concerns.
