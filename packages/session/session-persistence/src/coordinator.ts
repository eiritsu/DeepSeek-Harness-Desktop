...
export interface PersistenceBackend<TornMarker = unknown> {
  ...
  /**
   * Permanently remove one materialized session and its stored events.
   * @param id - persisted session identity.
   * @returns whether a stored session existed and was removed.
   */
...
export class PersistenceCoordinator<TornMarker = unknown> {
  ...
  /**
   * Delete one cold session after all earlier work for its id settles.
   * Lazy create intents resolve without touching the backend; every successful
   * path emits the post-delete event after in-memory state is invalidated.
  ...
  async delete(id: SessionId): Promise<void> {
    ...
    await this.serialize(id, async () => {
      ...
      if (!await this.backend.deleteStored(id)) {
