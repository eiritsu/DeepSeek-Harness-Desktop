...
| `delete(id): Promise<void>` | 永久移除一个冷 Session 身份。实时 Session 会被拒绝；尚未实体化的延迟创建 intent 会直接取消而不创建产物；未知 id 会被拒绝。成功后，后端和协调器状态均不再暴露该 Session，再 emit `session-persistence/deleted`。 |
...
| `deleteStored(id)` | 永久移除一份已实体化 Session，并报告它此前是否存在。协调器负责与同 id 的先前操作串行化，并独占公开 deleted 事件。 |
...
- **无保留策略或回收站**：`delete` 会立即永久生效；定时剪枝和可恢复删除仍由上层负责。
