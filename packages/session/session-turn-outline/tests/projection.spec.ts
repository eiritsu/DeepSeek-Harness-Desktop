...
describe('turn outline projection unit', () => {
  ...
  it('labels an edit-and-resend turn with its replayed prompt and keeps the turns it did not open', async () => {
    const { ctx, session } = await harness(true)
    const earlier = session.append('turn/start', { turn: 1 }).seq
    appendPrompt(session, '9/21 record work')
    appendAssistant(session, 1, 1, '9/21 answer')
    endTurn(session, 1)
    const aborted = session.append('turn/start', { turn: 2 }).seq
    const original = createUserMessage({
      content: [{ type: 'text', text: '发现一个问题哈' }],
      source: { kind: 'user' },
    })
    const originalSeq = session.append('user/message', original, { surfaceOp: 'append' }).seq
    const noticeSeq = session.append('user/message', createUserMessage({
      content: [{ type: 'text', text: '[model changed: assistant turns above this point were generated by glm-5.3-flashx]' }],
      source: { kind: 'plugin', plugin: 'model-selection', form: 'notice', summary: 'glm-5.3-flashx → glm-5.3-flash' },
    }), { surfaceOp: 'append' }).seq
    session.append('turn/end', { turn: 2, reason: { kind: 'aborted', reason: { kind: 'user' } } })
    const resend = session.append('turn/start', { turn: 3 }).seq
    session.append('user/message', createUserMessage({
      content: [{ type: 'text', text: '发现一个问题哈，我们这个会话是不是有记录丢失？' }],
      source: { kind: 'assistant-retry', retryOf: { kind: 'user-message', messageId: original.id } },
    }), {
      surfaceOp: { op: 'replace', startSeq: originalSeq, endSeq: noticeSeq },
      sourceEventSeqs: [originalSeq, noticeSeq],
    })
    appendAssistant(session, 3, 1, 'records are intact')
    appendPrompt(session, '但是这个会话中9月21日和9月22日的记录丢失了')
    endTurn(session, 3)

    expect(outlineOf(ctx, session)).toEqual([
      { turn: 1, seq: earlier, prompt: '9/21 record work', response: '9/21 answer' },
      { turn: 2, seq: aborted, prompt: '发现一个问题哈', response: '' },
      { turn: 3, seq: resend, prompt: '发现一个问题哈，我们这个会话是不是有记录丢失？', response: 'records are intact' },
    ])
  })
