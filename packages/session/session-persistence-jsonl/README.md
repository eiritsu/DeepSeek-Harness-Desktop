...
- **Deletion is permanent** — `delete` removes the fixed transcript and its now-empty session directory, then fsyncs the parent directory on POSIX; there is no trash or retention window.
