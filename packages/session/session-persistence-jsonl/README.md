...
- **Deletion removes every recognized generation** — `delete(id)` takes the backend write claim and cross-process lease, removes compressed and uncompressed generation logs, and retains the stable `session.lock` inode so another process cannot race through a replacement lock path.
