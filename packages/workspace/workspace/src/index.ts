...
export class WorkspaceRegistry extends Service {
  ...
  constructor(ctx: Context) {
    ...
    ctx.on('session-persistence/deleted', (sessionId) => {
      void this.removeDeletedSession(sessionId).catch((error: unknown) => {
        this.ctx.logger.warn(
          `workspace registry failed to remove deleted session '${sessionId}': ${String(error)}`,
        )
      })
    })
  }
  ...
  /** Remove a deleted identity from every account and the archive set. */
  private removeDeletedSession(id: SessionId): Promise<void> {
    return this.enqueueOperation(async () => {
      this.headers.delete(id)
      this.sessionPaths.delete(id)
      this.invalidSessionPaths.delete(id)
      for (const entity of this.entities.values()) await entity.detachSession(id)
      const state = this.requireState()
