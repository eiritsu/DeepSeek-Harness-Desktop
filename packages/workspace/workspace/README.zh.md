...
- 会话删除与破坏性的文件夹移除仍是彼此独立的能力；删除 Workspace 注册记录绝不能替代二者（参见[决策记录](../../../.agents/notes/implemented/feature/2026-07-27-workspace-registration-deletion.zh.md)）。`session-persistence/deleted` 事件会立即从所有 workspace 记账和全局归档集合中移除已删 id。
- 头部索引会在启动时刷新，也会在 attach 必须解析未缓存持久 id 时刷新；另一进程造成的 cwd 损坏或变更会在下次刷新或重启后被发现。
