  ...
  },
  {
    ...
    methods: [
      ...
      },
      {
        ...
        description: 'Deployment-resolved generic file policy used by authoritative and fast-path validation.',
        parameters: [],
      },
      {
      ...
      {
        signature: 'async recognizeFile(ref: FileAttachmentRef, signal?: AbortSignal): Promise<FileRecognitionResult | undefined>',
        description: 'Read one file and ask the first supporting recognizer for semantic text.',
        parameters: [{ name: 'ref', description: 'durable file reference selected for recognition.' }, { name: 'signal', description: 'optional cancellation for read and recognition work.' }],
        returns: 'bounded recognized text, or undefined when no recognizer supports or accepts the file.',
      },
      {
        signature: 'async recognizeImage(ref: ImageAttachmentRef, signal?: AbortSignal): Promise<FileRecognitionResult | undefined>',
        description: 'Read one normalized image and ask the first supporting recognizer for semantic text.',
        parameters: [{ name: 'ref', description: 'durable image selected for recognition.' }, { name: 'signal', description: 'optional cancellation for read and recognition work.' }],
      ...
      },
      {
        ...
        parameters: [{ name: 'input', description: 'raw bytes, media type, and optional display name.' }],
      ...
      },
      {
        ...
        description: 'Read one generic file and verify its digest and recorded metadata.',
        parameters: [{ name: 'ref', description: 'durable reference from the session log.' }, { name: 'signal', description: 'optional cancellation.' }],
  ...
  },
  {
    ...
    methods: [
      ...
      },
      {
        ...
        description: 'Register one ordered plugin that fills metadata missing from model discovery results. Provider-disclosed values and earlier enrichers win; patches for ids absent from the discovery result are ignored.',
        parameters: [{ name: 'enrich', description: 'receives detached candidates and returns metadata patches.' }],
      ...
      },
      {
        ...
        description: 'Register one ordered exact-model input resolver. Adapters consult this according to their own explicit-configuration precedence; the first resolver with an answer wins.',
        parameters: [{ name: 'resolve', description: 'exact route/model lookup.' }],
        returns: 'disposer withdrawing only this resolver.',
      },
      {
        signature: 'async resolveModelInput( provider: string, model: string, signal?: AbortSignal, ownedBy?: string, baseURL?: string, ): Promise<readonly ModelModality[] | undefined>',
        description: 'Resolve exact input modalities from registered external catalogs.',
        parameters: [{ name: 'provider', description: 'configured provider route.' }, { name: 'model', description: 'exact model id sent to the provider.' }, { name: 'signal', description: 'cancellation for asynchronous catalog access.' }, { name: 'ownedBy', description: 'upstream model owner preserved from discovery, when disclosed.' }, { name: 'baseURL', description: 'exact provider endpoint used by the configured route, when available.' }],
      ...
      },
      {
        ...
        description: 'Register one ordered exact-model capacity resolver. External catalog capacities replace adapter catalog values because they may describe models released after the installed adapter dependency.',
        parameters: [{ name: 'resolve', description: 'exact route/model lookup.' }],
        returns: 'disposer withdrawing only this resolver.',
      },
      {
        ...
        description: 'Resolve exact capacities from registered external catalogs.',
        parameters: [{ name: 'provider', description: 'configured provider route.' }, { name: 'model', description: 'exact model id sent to the provider.' }, { name: 'signal', description: 'cancellation for asynchronous catalog access.' }, { name: 'ownedBy', description: 'upstream model owner preserved from discovery, when disclosed.' }, { name: 'baseURL', description: 'exact provider endpoint used by the configured route, when available.' }],
        returns: 'the first resolver answer, or `undefined` when none knows the model.',
      },
      {
  ...
  },
  {
    ...
    methods: [
      ...
      },
      {
        ...
        description: 'Permanently delete one session\'s stored log. Operations for the same id remain serialized with appends and recovery. A live Session rejects; callers must first stop and dispose its owning lifecycle. An unknown id rejects, while a lazy create intent with no artifact is cancelled.',
...
export const EVENT_API: readonly EventApiEntry[] = [
  ...
  },
  {
    ...
    description: 'Emitted after one session identity has left durable storage. Consumers use this edge to remove derived records; the deleted log cannot be read again when listeners run.',
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'FileRecognitionResult',
    declaration: 'export interface FileRecognitionResult {\n    text: string;\n}',
  },
  {
    name: 'FileRecognizer',
    declaration: 'export interface FileRecognizer {\n    id: string;\n    supports(ref: FileAttachmentRef | ImageAttachmentRef): boolean;\n    recognize(file: StoredFileAttachment | StoredImageAttachment, signal?: AbortSignal): Promise<FileRecognitionResult | undefined>;\n}',
  },
  {
  ...
  {
    name: 'LlmModelCapacityRequest',
    declaration: 'export interface LlmModelCapacityRequest {\n    provider: string;\n    model: string;\n    ownedBy?: string;\n    baseURL?: string;\n    signal?: AbortSignal;\n}',
  },
  {
    name: 'LlmModelCapacityResolver',
    declaration: 'export interface LlmModelCapacityResolver {\n    (request: LlmModelCapacityRequest): Promise<LlmModelCapacity | undefined>;\n}',
  },
  {
