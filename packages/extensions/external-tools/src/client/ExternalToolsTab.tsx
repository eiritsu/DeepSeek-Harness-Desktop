...
export function ExternalToolsTab(props: ExternalToolsTabProps) {
  ...
  return <div className={css.root}>
    ...
    {state.rows.map(row => <section className={css.card} key={row.entry.id}>
      <div className={css.header}><div><h3>{row.entry.displayName}</h3><p>{props.t(row.entry.descriptionKey)}</p></div><label className={css.toggle}><input type="checkbox" checked={row.enabled} onChange={() => { void props.toggle(row.entry.id) }} /><span>{props.t('externalToolsEnable')}</span></label></div>
