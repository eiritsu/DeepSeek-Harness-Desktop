...
Use Cua Driver to inspect and operate desktop windows without installing its separate CLI or application. The native npm dependency runs inside the DSH host and exposes Cua Driver's own tools. Screenshots reach image-capable models through durable attachments. This stable package in the computer-use group requires the launching host's desktop permissions. The Desktop and Lite products mount it by default; other profiles select it explicitly.
...
- **Upstream schema drift** — tool schemas follow the pinned upstream SDK version and change only when that dependency version changes.
