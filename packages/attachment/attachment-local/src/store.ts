...
function ensureReference(ref: Pick<AttachmentRef, 'attachmentId'>): string {
...
export async function commitPreparedImageFile(
  ...
  await commitPreparedAttachment(root, prepared.data, prepared.ref)
  return prepared.ref
}

/** Publish verified bytes under their content-addressed reference. */
 ...
 * @param input - raw file bytes and display metadata.
...
/**
 * Read and verify one generic file without interpreting its format.
 * @param root - absolute attachment storage root.
