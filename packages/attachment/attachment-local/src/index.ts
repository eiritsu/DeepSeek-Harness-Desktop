...
/** Default maximum bytes for one generic file upload. */
export const DEFAULT_MAX_FILE_BYTES = 32 * 1024 * 1024
/** Default maximum generic files in one submitted message. */
export const DEFAULT_MAX_FILES_PER_MESSAGE = 20
/** Default maximum aggregate generic file bytes in one submitted message. */
...
export class LocalAttachmentStore extends AttachmentStore {
  ...
  async saveFile(input: SaveFileAttachment): Promise<FileAttachmentRef> {
    if (input.data.byteLength > this.fileLimits.maxFileBytes) {
      throw new AttachmentError('File exceeds the configured byte limit.', 'FILE_TOO_LARGE')
    }
    return saveFileAttachment(this.root, input)
  }
