...
describe('local attachment service', () => {
  ...
  it('stores arbitrary files byte-for-byte through the service boundary', async () => {
    ...
    try {
      ...
      const ref = await service.saveFile({
        ...
        name: '/private/input/sample.anything',
      })
      expect(ref).toMatchObject({ mediaType: 'application/x-custom', bytes: 4, name: 'sample.anything' })
