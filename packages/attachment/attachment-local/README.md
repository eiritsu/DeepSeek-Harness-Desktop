...
The private local implementation of [`@deepseek-ai/dsh-attachment`](../attachment). Arbitrary files and normalized images share immutable objects at `<DSH_HOME>/attachments/v1/objects/<sha256-prefix>/<sha256>` and are addressed by an opaque `sha256:` id. Each process proves a home durable once by syncing every ancestor entry to the filesystem root. Writes use a private staging directory, owner-only files, a synced temporary file, an atomic exclusive hard-link publish, and directory syncs on the publication path (POSIX; Windows relies on filesystem metadata journaling) so the reported reference survives a crash.

Generic file admission defaults to 32 MiB per file, 20 files per message, and 100 MiB total. Files retain their exact bytes and a path-stripped display name. The service does not infer semantics from the extension; optional recognizer plugins own content extraction.
