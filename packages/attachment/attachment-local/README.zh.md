...
这是 [`@deepseek-ai/dsh-attachment`](../attachment) 的私有本地实现。任意文件与规范化图片共享 `<DSH_HOME>/attachments/v1/objects/<sha256-prefix>/<sha256>` 下的不可变对象，并通过不透明的 `sha256:` 标识符寻址。每个进程都会把每级祖先目录项同步到文件系统根目录，以此一次性证明 home 已持久化。写入使用私有暂存目录、仅所有者可访问的文件、经过同步的临时文件、原子且排他的硬链接发布，并对发布路径执行目录同步（适用于 POSIX；Windows 依赖文件系统元数据日志），确保已报告的引用能够在崩溃后继续存在。

通用文件准入默认限制为单文件 32 MiB、每条消息 20 个文件、总计 100 MiB。文件保留原始字节和已去除路径的展示名称。服务不根据扩展名推断语义；内容提取由可选识别器插件负责。
