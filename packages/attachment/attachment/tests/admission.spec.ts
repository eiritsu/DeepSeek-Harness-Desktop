...
import { admitEncodedFiles, admitEncodedImages, UNKNOWN_FILE_MEDIA_TYPE } from '@deepseek-ai/dsh-attachment'
import type {
  FileAttachmentRef, ImageAttachmentRef, SaveFileAttachment, SaveImageAttachment,
...
describe('admitEncodedFiles', () => {
  it('accepts empty files, normalizes an empty media type, and preserves order', async () => {
    const { store, mocks } = storeOf()
    const refs = await admitEncodedFiles(store, [
      { mediaType: '', data: '', name: 'empty.bin' },
      { mediaType: 'application/pdf', data: PNG, name: 'brief.pdf' },
    ])
    expect(mocks.saveFiles).toHaveBeenCalledTimes(1)
    expect(mocks.saveFiles.mock.calls[0]?.[0].map(input => [input.name, input.mediaType, input.data.byteLength]))
      .toEqual([
        ['empty.bin', UNKNOWN_FILE_MEDIA_TYPE, 0],
        ['brief.pdf', 'application/pdf', 3],
      ])
    expect(refs.map(ref => ref.attachmentId)).toEqual(['file-1', 'file-2'])
  })

  it('rejects non-canonical base64 before storage', async () => {
    const { store, mocks } = storeOf()
    await expect(admitEncodedFiles(store, [{ mediaType: 'application/octet-stream', data: 'AAA' }]))
