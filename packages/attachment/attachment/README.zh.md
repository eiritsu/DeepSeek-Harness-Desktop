...
你可以把图片和通用文档附加到提示词中。图片会被规范化；通用文件按原字节持久化，并由已注册 recognizer 转换为有界文本，因此 JSON 与源码文件在会话重放时仍可用。随附的 `dsh` 组合无需配置即可支持这一点。浏览器路径、提供方 URL、本地存储路径与 base64 绝不会进入持久会话事件。未发送的草稿在提交前仍留在浏览器中，已存储对象不会自动删除，音频/视频识别需要配置外部端点。
...
通用文件通过 `saveFiles` 接收，并保存在同一内容寻址附件存储中。首个 `supports` 方法接受持久引用的 recognizer 会为 `file` 内容块返回语义文本；不受支持的文件仍会持久化，后续消费者可以继续使用其引用。
...
- **通用文件会以文本投影给模型**——原始字节和引用仍然持久化；除非 provider 提供原生文件表示，否则模型接收识别文本。
...
角色无关的 `ImageBlock` 把助手侧图片输出留作前瞻兼容；当前生产适配器声明只输出文本，因此只有用户内容携带图片。
