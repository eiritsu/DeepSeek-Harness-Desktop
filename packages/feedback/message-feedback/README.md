...
- **Out-of-band log removal can leave an orphan** — `session-persistence/deleted` removes the Session's sidecar row after an authoritative delete, while `session/disposed`/`host/session-removed` remain detach-only. Manual storage removal emits no event and can still leave a lifecycle-guarded orphan.
