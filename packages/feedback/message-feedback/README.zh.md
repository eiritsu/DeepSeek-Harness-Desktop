...
- **带外移除日志仍可能留下遗留记录**——权威删除后的 `session-persistence/deleted` 会移除该 Session 的 sidecar 行，`session/disposed`/`host/session-removed` 仍只表示 detach。手工删除存储不会 emit 事件，因此仍可能留下受生命周期身份保护的遗留记录。
