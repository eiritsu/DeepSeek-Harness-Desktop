...
describe('MessageFeedbackService public contract', () => {
  ...
  it('removes the lifecycle sidecar after durable Session deletion', async () => {
    const { ctx, persistence } = await harness()
    const fixture = messageFixture('deleted-sidecar')
    ...
    const table = (ctx.messageFeedback as unknown as {
      readonly table: KvTable<SessionId, unknown>
    }).table
    expect(table.get(fixture.session.id)).toBeDefined()

    await persistence.delete(fixture.session.id)

    await vi.waitFor(() => { expect(table.get(fixture.session.id)).toBeUndefined() })
  })
