...
declare module '@deepseek-ai/dsh-session-projection/types' {
  ...
  interface SessionProjectionMap {
    ...
    /**
     * The deployment's generic-file intake limits. Clients use these values
     * for count and byte pre-checks; key absence means no attachment service.
     */
...
export interface SessionsApi {
  ...
  /**
   * Permanently deletes one Session log. A running Session rejects. Children
   * require `recursive: true`; recursive deletion commits leaves before their
   * ancestors and returns every removed identity.
   */
  ...
  Promise<RpcResponse<{ attachment: AttachmentRef; data: string }>>
