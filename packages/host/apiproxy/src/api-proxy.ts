...
import { AttachmentError, admitEncodedFiles, admitEncodedImages, nativeFileModality } from '@deepseek-ai/dsh-attachment'
...
/** Validate one prompt before publishing model-visible durable attachment references. */
async function durablePromptContent(
  ...
  const imageRecognition = inputModalities !== undefined && !inputModalities.includes('image')
    ? await Promise.all(imageRefs.map(ref => ctx.attachments.recognizeImage(ref)))
    : []
  const fileRecognition = await Promise.all(fileRefs.map(async (ref) => {
    const modality = nativeFileModality(ref)
    return modality !== undefined && inputModalities?.includes(modality) === true
      ? undefined
      : ctx.attachments.recognizeFile(ref)
  }))
  let nextImage = 0
  let nextFile = 0
  return content.map((part) => {
    switch (part.type) {
      ...
      case 'image': {
        ...
        const recognized = imageRecognition[index]
        return {
          type: 'image',
          attachment: imageRefs[index] as ImageAttachmentRef,
          ...(recognized === undefined || recognized.text === '' ? {} : { recognizedText: recognized.text }),
        }
      }
      case 'file': {
        const index = nextFile++
        const recognized = fileRecognition[index]
        return {
          type: 'file',
          attachment: fileRefs[index] as FileAttachmentRef,
          ...(recognized === undefined || recognized.text === '' ? {} : { recognizedText: recognized.text }),
        }
      }
    }
...
type ReferencedAttachment =
  ...
  | { kind: 'file'; ref: FileAttachmentRef }

/** Search durable content for an attachment reference, including nested tool results. */
function attachmentBlockIn(content: unknown, attachmentId: string): ReferencedAttachment | undefined {
  ...
  for (const value of content) {
    ...
    if (typeof block.attachment === 'object' && block.attachment !== null) {
      if (block.type === 'image') {
        ...
        if (String(ref.attachmentId) === attachmentId) return { kind: 'image', ref }
      }
      if (block.type === 'file') {
        const ref = block.attachment as FileAttachmentRef
        if (String(ref.attachmentId) === attachmentId) return { kind: 'file', ref }
      }
    }
...
function attachmentInEvent(event: SessionEvent, attachmentId: string): ReferencedAttachment | undefined {
  ...
  if (data.message !== undefined) {
    const wrapped = attachmentBlockIn(data.message.content, attachmentId)
    if (wrapped !== undefined) return wrapped
  }
  if (data.inserted !== undefined) {
    for (const message of data.inserted) {
      const inserted = attachmentBlockIn(message.content, attachmentId)
      if (inserted !== undefined) return inserted
    }
  }
  if (event.type === 'assistant/chunk' && data.chunk?.type === 'block-end') {
    return attachmentBlockIn([data.chunk.block], attachmentId)
  }
  return undefined
}

/** Resolve the first reference matching one opaque id. */
function referencedAttachment(events: readonly SessionEvent[], attachmentId: string): ReferencedAttachment | undefined {
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  /** Consumer teardown capabilities for ordinary Agents this gateway created or resumed. */
  ...
  const rememberAgentHandle = (handle: AgentHandle): Agent => {
  ...
  return {
    sessions: {
      ...
      async delete(request) {
        ...
        let persistedIds: ReadonlySet<SessionId>
        try {
          const persisted = await persistence.list()
          persistedIds = new Set(persisted.map(header => header.id))
          const byId = new Map(persisted.map(header => [header.id, header]))
          for (const session of ctx.sessions.list()) byId.set(session.id, session.header)
          headers = [...byId.values()]
        } catch (error: unknown) {
          return err(request, {
            code: 'internal',
            message: `failed to inspect session lineage before deletion: ${String(error)}`,
            details: {},
          })
        }
        ...
        const targets = new Set<SessionId>([sessionId])
        for (;;) {
          const before = targets.size
          for (const header of headers) {
            if (header.parentSession !== undefined && targets.has(header.parentSession)) targets.add(header.id)
          }
          if (targets.size === before) break
        }
        const childSessionIds = [...targets].filter(id => id !== sessionId)
        if (childSessionIds.length > 0 && !recursive) {
          return err(request, {
            code: 'session-has-children',
            message: `session "${sessionId}" has ${childSessionIds.length} descendant session(s)`,
            details: { sessionId, childSessionIds },
          })
        }
        const depth = (id: SessionId): number => {
          let current = headers.find(header => header.id === id)
          let value = 0
          while (current?.parentSession !== undefined && targets.has(current.parentSession)) {
            value += 1
            current = headers.find(header => header.id === current?.parentSession)
          }
          return value
        }
        const ordered = [...targets].sort((left, right) => depth(right) - depth(left))
        for (const id of ordered) {
          const live = ctx.agents.get(id)
          if (live === undefined && ctx.sessions.get(id) === undefined) continue
          if (live?.status === 'running') {
            return err(request, {
              code: 'session-running',
              message: `stop session "${id}" before deleting it`,
              details: { sessionId: id },
            })
          }
          const handle = ownedAgentHandles.get(id)
          if (handle === undefined || handle.agent !== live) {
            return err(request, {
              code: 'agent-busy',
              message: `session "${id}" is live under another owner and cannot be deleted here`,
              details: { reason: 'dispose the owning agent before deleting its session' },
            })
          }
        }
        try {
          for (const id of ordered) await ownedAgentHandles.get(id)?.dispose()
          ...
          for (const id of ordered) {
            try {
              ...
              if (!persistedIds.has(id) && String(error).endsWith('not found')) {
                ctx.emit('session-persistence/deleted', id)
                deletedSessionIds.push(id)
                continue
              }
              if (recursive && String(error).includes(`session "${id}" not found`)) continue
              throw error
            }
          }
      ...
      async prompt(request) {
        ...
        const hasAttachment = hasImage || content.some(part => part.type === 'file')
        const admit = async (): Promise<RpcResponse<{ accepted: true }>> => {
          try {
            const current = selectionFor(agent).current
            const inputModalities = hasAttachment
              ? (await ctx.llm.resolveModelInfo(current.provider, current.model)).inputModalities
              : undefined
            const durable = await durablePromptContent(ctx, content, inputModalities)
            if (inputModalities !== undefined
              && !inputModalities.includes('image')
              && durable.some(block => block.type === 'image'
                && (block.recognizedText === undefined || block.recognizedText === ''))) {
              return err(request, {
            ...
            const unsupportedNativeFile = durable.find((block) => {
              if (block.type !== 'file') return false
              const modality = nativeFileModality(block.attachment)
              return modality !== undefined
                && inputModalities?.includes(modality) !== true
                && (block.recognizedText === undefined || block.recognizedText === '')
            })
            if (unsupportedNativeFile?.type === 'file') {
              const modality = nativeFileModality(unsupportedNativeFile.attachment)
              return err(request, {
                code: 'attachment-error',
                message: `Model "${current.model}" cannot receive ${modality ?? 'file'} input and no recognizer produced text.`,
                details: { reason: 'MODEL_DOES_NOT_SUPPORT_FILE_MODALITY' },
              })
            }
      ...
      async attachment(request) {
        ...
        const referenced = referencedAttachment(state.events, String(attachmentId))
        ...
        try {
          const stored = referenced.kind === 'image'
            ? await ctx.attachments.readImage(referenced.ref)
            : await ctx.attachments.readFile(referenced.ref)
          return ok(request, {
