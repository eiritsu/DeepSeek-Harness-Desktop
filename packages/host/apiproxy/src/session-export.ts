 ...
 * sessions' stored artifact text verbatim plus every referenced attachment object.
 * The root artifact sits under its original base name (`session.jsonl`); each
 * subagent descendant under `subagents/<id>/<filename>`; each image under
 * `media/<attachmentId>.<ext>` and each generic file under
 * `files/<attachmentId>/<name>` (content-addressed, so one archive never
...
/** Safe leaf name used for one generic attachment in an export archive. */
function fileEntryPath(ref: FileAttachmentRef): string {
  const raw = ref.name ?? 'attachment.bin'
  const leaf = raw.slice(Math.max(raw.lastIndexOf('/'), raw.lastIndexOf('\\')) + 1)
  const safe = leaf.replace(/[^A-Za-z0-9._ -]/g, '_').replace(/^\.+$/, '_').slice(0, 255) || 'attachment.bin'
  return `files/${String(ref.attachmentId)}/${safe}`
}
...
function collectAttachmentRefs(
  ...
  while (pending.length > 0) {
    ...
    if (block.type === 'file' && typeof block.attachment === 'object' && block.attachment !== null) {
      ...
      files.set(String(ref.attachmentId), ref)
    }
 ...
 * after the consumer moves on), then every distinct attachment object referenced by any of
...
/** How many binary attachment bytes one zip push carries before checking response capacity. */
 ...
 * @param data - the stored attachment bytes.
