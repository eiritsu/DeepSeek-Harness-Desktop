...
function fileEventLine(id: string, name: string): string {
  return `{"type":"user/message","seq":2,"time":1000,"data":{"content":[{"type":"file","attachment":{"attachmentId":"${id}","mediaType":"application/vnd.openxmlformats-officedocument.wordprocessingml.document","bytes":3,"name":${JSON.stringify(name)}}}]}}`
}
...
async function buildApi(
  ...
  services: {
    ...
    readFile?: (ref: FileAttachmentRef, signal?: AbortSignal) => Promise<{ ref: FileAttachmentRef; data: Uint8Array }>
    sessions?: {
  ...
  if (services.attachments !== false) {
    ...
    ctx.provide('attachments', {
      ...
      saveFile: async () => { throw new Error('export never saves files') },
      readImage,
      readFile: services.readFile ?? (async (ref: FileAttachmentRef) => ({ ref, data: new Uint8Array(ref.bytes) })),
    } as never)
  }
...
describe('session.export download endpoint', () => {
  ...
  it('exports generic files byte-for-byte under a sanitized original leaf name', async () => {
    const bytes = Uint8Array.of(7, 8, 9)
    const readFile = vi.fn(async (ref: FileAttachmentRef) => ({ ref, data: bytes }))
    const root = artifact('session-root', undefined, fileEventLine('file-one', '../../Quarterly?.docx') + '\n')
    const api = await buildApi({ 'session-root': root }, [], { readFile })
    const response = await toFetchHandler(api).fetch(
      new Request('http://host/api/session.export?sessionId=session-root'),
    )
    const files = unzipSync(await responseBytes(response))
    expect(files['files/file-one/Quarterly_.docx']).toEqual(bytes)
