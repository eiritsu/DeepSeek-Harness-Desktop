...
describe('session.history projections block', () => {
  ...
  it('publishes attachment intake limits as constant units while both seams are composed', async () => {
    const { ctx, session } = await harness(true)
    const limits = {
    ...
    const fileLimits = { maxFileBytes: 7, maxFilesPerMessage: 3, maxMessageFileBytes: 11 }
    ...
    expect(response.result.value.projections?.values['fileLimits']).toEqual(fileLimits)
    ...
    expect(frames.some(f => f.type === 'session/projection' && f.key === 'fileLimits')).toBe(false)
  })
  ...
  it('leaves the imageLimits key absent while no attachment service is composed', async () => {
    ...
    expect('fileLimits' in (response.result.value.projections?.values ?? {})).toBe(false)
  })
