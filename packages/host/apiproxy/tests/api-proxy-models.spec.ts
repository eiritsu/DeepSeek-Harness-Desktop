...
function registerAudioCapable(ctx: Context): void {
  ctx.llm.registerAdapter(['audio-native'], new class extends CatalogAdapter {
    override resolveModel(provider: string, model: string): Promise<LlmResolvedModelInfo> {
      return Promise.resolve({ provider, id: model, name: model, inputModalities: ['text', 'audio'] })
    }
  }('Audio Native', []))
}
...
describe('Web session model selection', () => {
  ...
  it('stores arbitrary file formats and records recognizer text in the durable block', async () => {
    const { ctx, agent, sessionId } = await harness()
    const saveFiles = vi.fn((inputs: readonly { data: Uint8Array; mediaType: string; name?: string }[]) => Promise.resolve(
      inputs.map((input, index) => ({
        attachmentId: `file-${String(index + 1)}`,
        mediaType: input.mediaType,
        bytes: input.data.byteLength,
        ...input.name === undefined ? {} : { name: input.name },
      })),
    ))
    const recognizeFile = vi.fn(() => Promise.resolve({ text: 'recognized document text' }))
    ctx.provide('attachments', {
      fileLimits: { maxFileBytes: 16, maxFilesPerMessage: 2, maxMessageFileBytes: 24 },
      saveFiles,
      recognizeFile,
    } as never)
    const followup = vi.fn()
    ...
    const result = await api.sessions.prompt(request({
      ...
      mode: 'queue' as const,
      content: [
        { type: 'file' as const, mediaType: '', data: 'AQID', name: 'sample.unknown' },
        { type: 'text' as const, text: 'inspect' },
      ],
    }))
    expect(result.result.ok).toBe(true)
    expect(saveFiles.mock.calls[0]?.[0]).toMatchObject([{
      mediaType: 'application/octet-stream', name: 'sample.unknown', data: Uint8Array.of(1, 2, 3),
    }])
    expect((followup.mock.calls[0]?.[0] as UserMessage).content).toEqual([
      {
        type: 'file',
        attachment: {
          attachmentId: 'file-1', mediaType: 'application/octet-stream', bytes: 3, name: 'sample.unknown',
        },
        recognizedText: 'recognized document text',
      },
  ...
  it('uses native audio input when the selected route declares transport support', async () => {
    const { ctx, agent, sessionId } = await harness()
    registerAudioCapable(ctx)
    const ref = {
      attachmentId: 'native-audio',
      mediaType: 'audio/mpeg',
      bytes: 3,
      name: 'voice.mp3',
    }
    const recognizeFile = vi.fn(() => Promise.resolve({ text: 'external transcript' }))
    ctx.provide('attachments', {
      fileLimits: { maxFileBytes: 16, maxFilesPerMessage: 2, maxMessageFileBytes: 24 },
      saveFiles: () => Promise.resolve([ref]),
      recognizeFile,
    } as never)
    const followup = vi.fn()
    Object.assign(agent, { followup })
    const api = createApiProxy(ctx, {
      defaultModelSelection: () => ({ provider: 'audio-native', model: 'audio-chat' }),
      cwd: '/tmp',
    })
    ...
    const result = await api.sessions.prompt(request({
      sessionId,
      mode: 'queue' as const,
      content: [{ type: 'file' as const, mediaType: 'audio/mpeg', data: 'AQID', name: 'voice.mp3' }],
    }))

    expect(result.result).toEqual({ ok: true, value: { accepted: true } })
    expect(recognizeFile).not.toHaveBeenCalled()
    expect((followup.mock.calls[0]?.[0] as UserMessage).content).toEqual([{
      type: 'file',
      attachment: ref,
    }])
  ...
  it('uses Deepseek-Files transcription when the selected route lacks native audio', async () => {
    const { ctx, agent, sessionId } = await harness()
    registerTextOnly(ctx)
    const ref = {
      attachmentId: 'fallback-audio',
      mediaType: 'audio/mpeg',
      bytes: 3,
      name: 'voice.mp3',
    }
    ...
    expect(result.result).toEqual({ ok: true, value: { accepted: true } })
    expect(recognizeFile).toHaveBeenCalledWith(ref)
    expect((followup.mock.calls[0]?.[0] as UserMessage).content).toEqual([{
      type: 'file',
      attachment: ref,
      recognizedText: 'external transcript',
    }])
    await ctx.fiber.dispose()
  })

  it('rejects unsupported audio when Deepseek-Files produces no transcript', async () => {
    const { ctx, agent, sessionId } = await harness()
    registerTextOnly(ctx)
    const ref = {
      attachmentId: 'unrecognized-audio',
      mediaType: 'audio/mpeg',
      bytes: 3,
      name: 'voice.mp3',
    }
    ctx.provide('attachments', {
      fileLimits: { maxFileBytes: 16, maxFilesPerMessage: 2, maxMessageFileBytes: 24 },
      saveFiles: () => Promise.resolve([ref]),
      recognizeFile: () => Promise.resolve(undefined),
    } as never)
    const followup = vi.fn()
    Object.assign(agent, { followup })
    const api = createApiProxy(ctx, {
    ...
    expect(result.result).toMatchObject({
      ok: false,
      error: { code: 'attachment-error', details: { reason: 'MODEL_DOES_NOT_SUPPORT_FILE_MODALITY' } },
    })
    expect(followup).not.toHaveBeenCalled()
    await ctx.fiber.dispose()
  })

  it('records OCR text for new images admitted to a text-only model', async () => {
    const { ctx, agent, sessionId } = await harness()
    registerTextOnly(ctx)
    const ref = {
    ...
    const saveImages = vi.fn((inputs: readonly { data: Uint8Array; mediaType: 'image/png'; name?: string }[]) => {
      void inputs
      return Promise.resolve([ref])
    })
    const recognizeImage = vi.fn(() => Promise.resolve({ text: 'recognized scan text' }))
    ctx.provide('attachments', { saveImages, recognizeImage } as never)
    const followup = vi.fn()
    Object.assign(agent, { followup })
    const api = createApiProxy(ctx, {
      defaultModelSelection: () => ({ provider: 'text-only', model: 'plain' }),
    ...
    const result = await api.sessions.prompt(request({
      ...
      mode: 'queue' as const,
      content: [
        { type: 'image' as const, mediaType: 'image/png' as const, data: 'AQ==', name: 'scan.png' },
    ...
    expect(saveImages.mock.calls[0]?.[0]).toMatchObject([{
      mediaType: 'image/png', name: 'scan.png', data: Uint8Array.of(1),
    }])
    expect(recognizeImage).toHaveBeenCalledWith(ref)
    expect((followup.mock.calls[0]?.[0] as UserMessage).content).toEqual([
      { type: 'image', attachment: ref, recognizedText: 'recognized scan text' },
      { type: 'text', text: 'read this' },
    ])
    await ctx.fiber.dispose()
  })

  it('rejects a new image when neither the model nor a recognizer can read it', async () => {
    const { ctx, agent, sessionId } = await harness()
    registerTextOnly(ctx)
    const ref = {
      attachmentId: 'unrecognized-image',
      mediaType: 'image/png' as const,
      bytes: 1,
      width: 1,
      height: 1,
    ...
    ctx.provide('attachments', {
      ...
      recognizeImage: () => Promise.resolve(undefined),
    } as never)
    const followup = vi.fn()
    Object.assign(agent, { followup })
    const api = createApiProxy(ctx, {
