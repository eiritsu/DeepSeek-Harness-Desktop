...
- **External metadata does not add models** — a side-loaded exact-model resolver may supply modalities for an already configured id, but it does not change route membership, capacities, protocol, or endpoint. A route gains a model only when its profile or installed pi-ai catalog names one.
