  ...
  config:
    providers:
      ...
      openai:
        ...
        maxRequestFileBytes: 134217728   # accumulated base64 native-file payload
        retryPolicy:
          mode: normal
          maxRetries: 3
          backoff:
...
`reasoningEfforts` configures model-specific wire spellings without changing the Composer menu. Every model receives `Default`, `Off`, `Low`, `High`, and `Max`; `Default` sends no per-session override, while an explicit level is attempted for every route. A mapping such as `high: high` passes the canonical name through and `max: ultra` renames it for a gateway with its own vocabulary. Keys come from pi-ai's wider level set (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`) because the route-level `reasoning` default may still use pi-ai-native levels. `false` preserves a non-reasoning descriptor for Default calls, while explicit standard selections remain best-effort requests that the provider may reject. An empty declaration is refused.

The declaration translates to pi-ai's `Model.reasoning` + `thinkingLevelMap`. Explicit standard selections temporarily enable reasoning on the detached request descriptor, retain a configured wire spelling, and fill an absent `max` spelling with `max`; this bypasses pi-ai's model-capability clamp without changing a `Default` request. `off` keeps its protocol-specific behavior: a declared value such as `off: none` reaches the wire, while a null or absent value lets pi-ai emit the selected protocol's disable form or omit its common reasoning option. Private gateways still use `compat.thinkingFormat` and `compat.supportsReasoningEffort` when pi-ai cannot infer their dialect.
...
An external exact-model capacity resolver may replace those materialized context and output capabilities at call preparation time. This lets a refreshed catalog describe a newly released model without waiting for the installed pi-ai version or rewriting saved settings. The resolved `maxOutputTokens` remains model capability metadata: only a profile entry that explicitly names `maxTokens` becomes the harness `defaultMaxTokens` request cap.

Request modalities resolve entry `input` → installed catalog entry → an external exact-model resolver registered with `LlmRuntime` → route `defaultInput` (default `[text]`). The external lookup runs only for a model that reached the route fallback, so an explicit per-model correction and pi-ai's installed metadata remain authoritative; its result is captured in the same prepared model descriptor used for admission and provider dispatch. A gateway whose still-unknown models all take images can declare `[text, image]` once at the route. An entry's empty list means the same as an absent one — a model accepting nothing states no answer, so resolution continues — while the route's may not be empty because nothing sits below it to answer instead.
...
The adapter exposes each configured route's models through `ctx.llm.listModels(provider)`. This is provider-neutral selector metadata read from the same pi-ai `Models` collection the request path uses, so discovery does not create a second model registry. `ctx.llm.resolveModelInfo(provider, model)` performs that exact descriptor lookup once and returns its identity, context window, and configured output cap; `LlmRuntime` attaches the fixed reasoning controls. A model's **configured** `maxTokens` becomes the seam's `defaultMaxTokens`, so a request that names no output cap carries the one the deployment chose; a value inherited from the installed catalog is the model's output *capability* and never becomes a request default on its own.

The profile `reasoning` value is the deployment default used only when the session keeps `Default`; omitting it preserves the provider default. Per-request `GenerateOptions.reasoningEffort` takes precedence. `Off`, `Low`, `High`, and `Max` are not capability claims: the adapter attempts them with pi-ai's protocol translation, and an endpoint that does not implement the selected value returns its normal provider error. This keeps model discovery, provider ownership, and model-name heuristics out of the control's availability.

Supported profile fields are `apiKeyEnv`, `displayName`, `api`, `baseURL`, `models`, `modelOverrides`, `compat`, `defaultContextWindow`, `defaultMaxTokens`, `defaultInput`, `headers`, `reasoning`, `thinkingBudgets`, `cacheRetention`, `transport`, `timeoutMs`, `websocketConnectTimeoutMs`, `streamIdleTimeoutMs`, `maxRequestImageBytes`, `maxRequestFileBytes`, `requestImagePixelBudget`, `requestImageMaxBytes`, and `retryPolicy`. Each resolved profile retry policy is captured with that provider route; omission uses the shared bounded normal default of five retries. The stream-idle interval is a positive finite Node timer delay, defaults to five minutes, and covers only an outstanding provider read, not consumer think time. Every image route derives a deterministic request version from the provider-independent normalized attachment under `requestImagePixelBudget` (default 2048 by 2048 total pixels) and `requestImageMaxBytes` (default 1MiB raw bytes). Before reading attachments, `maxRequestImageBytes` applies to conservative request-version upper bounds and replaces the oldest over-budget images with fixed text; exact base64 lengths are checked again after retained versions are generated. Google protocols also serialize catalog-declared `audio`, `video`, and `pdf` as inline media; `maxRequestFileBytes` bounds their accumulated base64 payload at 128MiB and replaces the oldest over-budget files with stable metadata text. Harness app attribution wins a conflicting configured header name.
...
Most listings disclose an id and nothing else; `context_window`/`context_length`, `max_output_tokens`/`max_tokens`, and an explicit top-level or `architecture.input_modalities` list are read when a gateway supplies them. Only supported `text` and `image` values are preserved, and an absent or unsupported-only list remains unknown rather than becoming a guessed capability. The Models page stores disclosed modalities on each adopted model automatically. Entries without a usable id are skipped rather than failing the whole listing. The reply is read under a four-megabyte ceiling enforced on the bytes actually received — the endpoint is a URL the user typed, so a declared length is checked first but never trusted as the bound. An unreachable endpoint, a refused credential, a non-JSON body, and a body with no `data` array all fail with `DISCOVERY_FAILED` and a message naming the endpoint and, for a 401 or 403 alone, the credential. Cancellation during the body read surfaces as `ABORTED`, like a cancellation before the request went out.

The listing's `owned_by` value is preserved as candidate metadata and stored on a model adopted by the Models page. The default `@deepseek-ai/dsh-model-catalog` Web Profile Bundle refreshes `models.dev` into a persisted last-good snapshot and uses a recognized owner plus the exact model id to fill the complete input declaration and current context/output capacities. A local owner alias can instead use an exact route `baseURL` match to a catalog provider API. Without either identity, each field requires identical same-id declarations; uncovered ids or fields use the installed pi-ai catalog under the same exact-match rules. It does not infer from the route, protocol, partial URL, or model-name pattern. This keeps the generic adapter independent from any one supplemental catalog; custom profiles can add the catalog as an ordinary Bundle.
...
The selected catalog model receives `GenerateOptions.system`, history, tools, and sampling fields supported by pi-ai's common streaming API. Each retained image is preceded by stable text naming its complete attachment id and actual request dimensions. Google protocols receive supported audio, video, and PDF files as native inline media; other protocols receive durable `Deepseek-Files` recognition text. Image and native-file payload bounds replace the oldest over-budget attachment with deterministic text before its bytes are read. Provider-native replay metadata is restored only when the adapter validates it for the historical content.
...
- **`maxRequestFileBytes` counts base64 native-file payload only** — the 128MiB default bounds accumulated audio, video, and PDF bytes independently from images; deployments must lower it below stricter provider request caps.
- **Catalog support does not invent a serializer** — dynamic `audio`, `video`, and `pdf` declarations become effective only for Google protocols whose pi-ai implementation emits arbitrary inline media. OpenAI-compatible routes remain `text/image` until their protocol-specific request formats are implemented.
...
- **External metadata does not add models** — side-loaded exact-model resolvers may supply modalities and capacities for an already configured id, but they do not change route membership, protocol, endpoint, or request defaults. A route gains a model only when its profile or installed pi-ai catalog names one.
