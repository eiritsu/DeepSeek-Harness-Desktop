...
点名了**已安装 catalog 所提供路由**的请求，直接由该 catalog 作答，完全不联网：pi-ai 的注册表才是它自家提供方的权威列表，且携带列表端点可能不会公布的上下文窗口、输出上限与输入模态。这类路由根本不需要 `baseURL`。只有 catalog 未描述的路由——网关、自建服务——才会经协议层询问；若它也没给端点，则会被告知去设置一个或手工填写模型。
...
所选 catalog 模型会收到 `GenerateOptions.system`、历史、工具，以及 pi-ai 通用流式 API 支持的采样字段。每张保留图片前都有稳定文本，写明完整附件 ID 和实际请求尺寸。Google 协议会把支持的音频、视频与 PDF 作为原生 inline media 发送；其他协议接收持久 `Deepseek-Files` 识别文本。图片与原生文件载荷上限会在读取附件字节前，把超预算的最旧附件替换成确定性文本。只有当适配器验证提供方原生回放元数据与历史内容匹配时，才会恢复这些元数据。
...
- **`maxRequestFileBytes` 只统计 base64 原生文件载荷**：128MiB 默认值独立限制累计音频、视频与 PDF 字节；部署必须把它调低到严格提供方的请求上限以内。
- **Catalog 支持不会凭空产生 serializer**：动态 `audio`、`video` 与 `pdf` 声明仅在 pi-ai 能发出任意 inline media 的 Google 协议上生效；OpenAI-compatible 路由在实现其协议专用请求格式前仍保持 `text/image`。
