...
const PI_AI_BINARY_APIS = new Set(['google-generative-ai', 'google-vertex'])

/** Modalities this pi-ai protocol implementation can serialize without preprocessing. */
function transportModalities(api: string): readonly ModelModality[] {
  return PI_AI_BINARY_APIS.has(api)
    ? ['text', 'image', 'audio', 'video', 'pdf']
    : ['text', 'image']
}

function effectiveModalities(api: string, declared: readonly ModelModality[]): ModelModality[] {
  const supported = transportModalities(api)
  return declared.filter(modality => supported.includes(modality))
}

function unsupportedFileModality(
  content: readonly ContentBlock[],
  inputModalities: readonly ModelModality[],
): ModelModality | 'file' | undefined {
  for (const block of content) {
    if (block.type === 'file') {
      const modality = nativeFileModality(block.attachment)
      if (modality === undefined || !inputModalities.includes(modality)) return modality ?? 'file'
    } else if (block.type === 'tool-result') {
      const nested = unsupportedFileModality(block.content, inputModalities)
...
function firstUnsupportedFileModality(
  messages: readonly Message[],
  inputModalities: readonly ModelModality[],
): ModelModality | 'file' | undefined {
  for (const message of messages) {
    const modality = unsupportedFileModality(message.content, inputModalities)
    if (modality !== undefined) return modality
  }
  return undefined
}
...
export interface PiAiAdapterOptions {
  ...
  /** Resolve complete exact modalities unless the per-model profile pins them. */
...
export class PiAiAdapter extends LlmAdapter {
  ...
  /** Resolve complete exact metadata unless the per-model profile pins it. */
  ...
  override async prepareCall(provider: string, model: string, signal?: AbortSignal): Promise<PreparedAdapterCall> {
    const snapshot = this.current()
    const resolvedModel = await this.modelForCall(snapshot, provider, model, signal)
    return {
      model: this.modelInfo(snapshot, provider, resolvedModel),
  ...
  private async * streamWithSnapshot(
    ...
    try {
      ...
      const unsupportedFile = firstUnsupportedFileModality(options.messages, resolved.inputModalities)
      if (unsupportedFile !== undefined) {
        throw new LlmError(`pi-ai model "${model.id}" cannot serialize ${unsupportedFile} file input`, 'UNSUPPORTED_CONTENT')
      }
      ...
      if ((containsImage || containsFile) && attachments === undefined) {
        throw new LlmError('pi-ai native attachment input requires the durable attachment service', 'UNSUPPORTED_CONTENT')
      }
