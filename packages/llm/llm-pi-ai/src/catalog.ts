...
/**
 * The request modalities pi-ai stores directly on a model descriptor. The
 * `Record` key type fails compilation when that upstream vocabulary changes.
 */
const PI_AI_MODALITY_GATE: Record<Model<Api>['input'][number], true> = {
  text: true,
  image: true,
}

/** Every models.dev request modality a profile may declare. */
export const MODALITIES = ['text', 'image', 'audio', 'video', 'pdf'] as const satisfies readonly PiAiModality[]

/** Reduce complete route metadata to the vocabulary pi-ai stores on its model descriptor. */
function piAiInput(input: readonly PiAiModality[]): Model<Api>['input'] {
  return input.filter((modality): modality is keyof typeof PI_AI_MODALITY_GATE => modality in PI_AI_MODALITY_GATE)
}
