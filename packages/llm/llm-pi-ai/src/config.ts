...
/** Default request-level bound on accumulated base64 generic-file payload. */
export const DEFAULT_MAX_REQUEST_FILE_BYTES = 128 * 1024 * 1024
 ...
 * model discovery preserves modalities a gateway explicitly reports, but
 * many OpenAI-compatible listings omit them, and the two wrong answers do not
 * cost the same. Under-claiming refuses the image before it is attached,
 * naming the model. Over-claiming admits one the provider then rejects
 * mid-turn, after the message is durable, leaving the session repeating a
...
export interface PiAiProviderProfile {
  ...
  /** Maximum accumulated base64 generic-file payload per request; oldest files become metadata text. */
...
export interface ResolvedPiAiProviderProfile
  ...
  maxRequestImageBytes: number
  /** Positive request-level base64 generic-file payload bound after defaulting. */
  maxRequestFileBytes: number
...
export function resolveProfiles(
  ...
  for (const [provider, source] of entries) {
    ...
    const maxRequestFileBytes = source.maxRequestFileBytes ?? DEFAULT_MAX_REQUEST_FILE_BYTES
    if (!Number.isInteger(maxRequestFileBytes) || maxRequestFileBytes <= 0) {
      throw new Error(`llm-pi-ai: provider "${provider}" maxRequestFileBytes must be a positive integer`)
    }
