 ...
 * its own providers, and it carries capacities and modalities a listing
 * endpoint may not disclose. Only a route the catalog does not describe — a
...
/** Supported request modalities explicitly disclosed by a listing entry. */
function inputModalities(...candidates: readonly unknown[]): ('text' | 'image')[] | undefined {
  for (const candidate of candidates) {
    if (!Array.isArray(candidate)) continue
    const modalities = [...new Set(candidate.filter((value): value is 'text' | 'image' => (
...
function readListing(body: unknown): LlmDiscoveredModel[] {
  ...
  for (const raw of data) {
    ...
    const ownedBy = label(entry?.owned_by)
    const name = label(entry?.name, entry?.display_name)
    const contextWindow = capacity(entry?.context_window, entry?.context_length)
    const maxTokens = capacity(entry?.max_output_tokens, entry?.max_tokens)
    const modalities = inputModalities(entry?.input_modalities, entry?.architecture?.input_modalities)
    models.push({
      id,
