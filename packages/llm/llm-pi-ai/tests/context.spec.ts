...
function projectionStore(
  ...
  readFile: AttachmentStore['readFile'] = vi.fn((value: FileAttachmentRef) => Promise.resolve({
    ref: value,
...
describe('pi-ai request context conversion', () => {
  ...
  it('resolves native audio files into binary pi-ai content for a capable protocol', async () => {
    const audio: FileAttachmentRef = {
    ...
    const context = await toPiContext(
      request([user([{ type: 'file', attachment: audio }])]),
      projectionStore(),
      undefined,
      undefined,
      undefined,
    ...
    expect(context.messages).toEqual([{
      role: 'user',
      content: [{ type: 'image', data: 'AQID', mimeType: 'audio/mpeg' }],
      timestamp: 0,
    }])
  })
