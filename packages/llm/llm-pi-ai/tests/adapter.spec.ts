...
import { DEFAULT_MAX_REQUEST_FILE_BYTES, DEFAULT_MAX_REQUEST_IMAGE_BYTES, resolveProfiles } from '../src/config.ts'
...
describe('PiAiAdapter provider routing', () => {
  ...
  it('resolves an attachment service mounted after the adapter when dispatching an image', async () => {
    ...
    class LateAttachmentStore extends AttachmentStore {
      readonly fileLimits = { maxFileBytes: 1, maxFilesPerMessage: 1, maxMessageFileBytes: 1 }
      ...
      saveFile(): Promise<never> { return Promise.reject(new Error('late fixture is image-only')) }
      readFile(): Promise<never> { return Promise.reject(new Error('late fixture is image-only')) }
...
describe('provider profile lifecycle', () => {
  ...
  it('validates empty, underspecified, legacy-shaped, and explicitly blank profiles', () => {
    ...
    expect(DEFAULT_MAX_REQUEST_FILE_BYTES).toBe(128 * 1024 * 1024)
    ...
    expect(() => resolveProfiles({ openai: { maxRequestFileBytes: 0 } })).toThrow(/maxRequestFileBytes/)
    expect(resolveProfiles({ openai: {} }).get('openai')?.maxRequestImageBytes)
      .toBe(DEFAULT_MAX_REQUEST_IMAGE_BYTES)
    expect(resolveProfiles({ openai: {} }).get('openai')?.maxRequestFileBytes)
      .toBe(DEFAULT_MAX_REQUEST_FILE_BYTES)
    expect(resolveProfiles({ openai: { maxRequestImageBytes: 1024 } }).get('openai')?.maxRequestImageBytes)
      .toBe(1024)
    expect(resolveProfiles({ openai: { maxRequestFileBytes: 2048 } }).get('openai')?.maxRequestFileBytes)
      .toBe(2048)
  })
  ...
  it('rejects invalid stream tunables at plugin load', async () => {
    const invalid = [
      ...
      { maxRequestFileBytes: 1.5 },
      { maxRequestFileBytes: Number.NaN },
    ]
  ...
  it('uses externally resolved modalities in the prepared adapter dispatch', async () => {
    const resolveInputModalities = vi.fn(() => Promise.resolve<readonly ('text' | 'image')[]>(['text', 'image']))
    ...
    const prepared = await adapter.prepareCall('a6', 'gemini-3.7-flash')
