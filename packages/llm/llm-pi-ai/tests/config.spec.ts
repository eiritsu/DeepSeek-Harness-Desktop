...
describe('modality schema boundary', () => {
  it('accepts catalog media modalities and rejects unknown values', () => {
    expect(configWith({ input: ['audio', 'video', 'pdf'] })).not.toThrow()
    expect(routeWith({ defaultInput: ['text', 'image', 'audio', 'video', 'pdf'] })).not.toThrow()
...
describe('request image policy bounds', () => {
  it.each([
    ...
    ['maxRequestFileBytes', 0, /maxRequestFileBytes must be a positive integer/],
    ['maxRequestFileBytes', 1.5, /maxRequestFileBytes must be a positive integer/],
