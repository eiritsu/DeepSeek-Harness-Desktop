...
async function harness(image?: StoredImageAttachment): Promise<Context> {
  ...
  if (image !== undefined) {
    ...
    class E2eAttachmentStore extends AttachmentStore {
      ...
      saveFile(): Promise<never> { return Promise.reject(new Error('e2e attachment fixture is image-only')) }
      readFile(): Promise<never> { return Promise.reject(new Error('e2e attachment fixture is image-only')) }
