...
export class LlmRuntime extends Service {
  ...
  registerModelDiscoveryEnricher(enrich: LlmModelDiscoveryEnricher): () => void {
    ...
    const dispose = this.ctx.effect(function* (this: LlmRuntime) {
      ...
      yield () => {
        this.discoveryEnrichers.delete(registration)
      }
  ...
  registerModelInputResolver(resolve: LlmModelInputResolver): () => void {
    ...
    const dispose = this.ctx.effect(function* (this: LlmRuntime) {
      ...
      yield () => {
        this.modelInputResolvers.delete(registration)
      }
  ...
  /** Detach provider-owned discovery metadata before crossing plugin registrations. */
  ...
  private async * adapterStream(
    ...
    try {
      ...
      const fileProjectedOptions = resolvedOptions.messages.some(message => contentHasFile(message.content))
        ? Object.isFrozen(resolvedOptions)
          ? deepFreeze({
            ...resolvedOptions,
            messages: projectFilesForModel(resolvedOptions.messages, modelInfo.inputModalities) as Message[],
          })
          : { ...resolvedOptions, messages: projectFilesForModel(resolvedOptions.messages, modelInfo.inputModalities) as Message[] }
        : resolvedOptions
      const projectedOptions = modelInfo.inputModalities !== undefined
        && !modelInfo.inputModalities.includes('image')
        && fileProjectedOptions.messages.some(message => contentHasImage(message.content))
        ? Object.isFrozen(fileProjectedOptions)
          ? deepFreeze({ ...fileProjectedOptions, messages: projectImagesForTextModel(fileProjectedOptions.messages) as Message[] })
          : { ...fileProjectedOptions, messages: projectImagesForTextModel(fileProjectedOptions.messages) as Message[] }
        : fileProjectedOptions
