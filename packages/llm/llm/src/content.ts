 ...
 * @param recognizedText - optional bounded OCR text recorded at admission.
...
export function textOnlyImageText(ref: ImageAttachmentRef, recognizedText?: string): string {
  ...
  const header = `[image represented as text because this model accepts text only; attachment sha256:${digest}]`
  return recognizedText === undefined || recognizedText === '' ? header : header + '\n' + recognizedText
}

/**
 * Stable model-facing description of one durable generic file.
 * @param ref - durable file reference projected into the request.
 * @param recognizedText - optional bounded text recorded at admission.
 * @returns deterministic metadata and recognized text.
 */
export function fileAttachmentText(ref: FileAttachmentRef, recognizedText?: string): string {
  const digest = String(ref.attachmentId).slice('sha256:'.length, 'sha256:'.length + 8)
  const name = ref.name ?? 'unnamed attachment'
  const header = '[attached file: ' + name + '; ' + ref.mediaType + '; ' + String(ref.bytes)
    + ' bytes; attachment sha256:' + digest + ']'
  return recognizedText === undefined || recognizedText === '' ? header : header + '\n' + recognizedText
}

/**
 * Test whether typed model content contains a generic file block.
 * @param content - typed content blocks, including nested tool results.
...
/** Replace files the selected route cannot transmit with deterministic model-facing text. */
function replaceFilesForModel(
  ...
  for (const [index, block] of blocks.entries()) {
    if (block.type === 'file') {
      ...
      if (modality !== undefined && inputModalities?.includes(modality) === true) {
        next?.push(block)
        continue
      }
    ...
    if (block.type === 'tool-result') {
      const content = replaceFilesForModel(block.content, inputModalities)
...
/**
 * Project durable generic files to provider-neutral text before adapter dispatch.
 * @param messages - provider-neutral messages containing durable file blocks.
 * @param inputModalities - exact modalities the selected route can serialize.
 * @returns the original messages when no file exists, otherwise messages with files projected to text.
 */
export function projectFilesForModel(
  ...
  const projected = messages.map((message) => {
    const content = replaceFilesForModel(message.content, inputModalities)
    return content === message.content ? message : { ...message, content }
  })
  return projected.some((message, index) => message !== messages[index]) ? projected : messages
}

function collectFileLengths(blocks: readonly ContentBlock[], lengths: number[]): void {
  for (const block of blocks) {
    if (block.type === 'file') lengths.push(base64Length(block.attachment.bytes))
    else if (block.type === 'tool-result') collectFileLengths(block.content, lengths)
  }
}

function replaceOldestFiles(blocks: readonly ContentBlock[], remaining: { count: number }): ContentBlock[] {
  let next: ContentBlock[] | undefined
  for (const [index, block] of blocks.entries()) {
    if (block.type === 'file' && remaining.count > 0) {
      remaining.count -= 1
      next ??= blocks.slice(0, index)
      next.push({ type: 'text', text: fileAttachmentText(block.attachment, block.recognizedText) })
      continue
    ...
    if (block.type === 'tool-result') {
      const content = replaceOldestFiles(block.content, remaining)
      if (content !== block.content) {
        next ??= blocks.slice(0, index)
        next.push({ ...block, content })
        continue
...
/**
 * Replace the oldest native files until their accumulated base64 payload fits one request.
 * @param messages - complete request history, oldest first.
 * @param maxRequestFileBytes - positive base64 payload bound; omission preserves every file.
 * @returns the original messages within the bound, otherwise shallow copies with stable file text.
 */
export function offloadRequestFiles(
  messages: readonly Message[],
  maxRequestFileBytes: number | undefined,
): readonly Message[] {
  if (maxRequestFileBytes === undefined) return messages
  const lengths: number[] = []
  for (const message of messages) collectFileLengths(message.content, lengths)
  let total = lengths.reduce((sum, bytes) => sum + bytes, 0)
  if (total <= maxRequestFileBytes) return messages
  let count = 0
  for (const bytes of lengths) {
    total -= bytes
    count += 1
    if (total <= maxRequestFileBytes) break
  }
  const remaining = { count }
  return messages.map((message) => {
    const content = replaceOldestFiles(message.content, remaining)
    return content === message.content ? message : { ...message, content }
  })
}
