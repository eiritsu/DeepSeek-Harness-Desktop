...
function image(bytes: number, recognizedText?: string): ContentBlock {
  return {
    type: 'image',
    attachment: {
...
function file(recognizedText?: string): ContentBlock {
  return {
    type: 'file',
    attachment: {
...
describe('projectFilesForModel', () => {
  it('returns file-free history unchanged', () => {
    const messages = [createUserMessage({ content: [{ type: 'text', text: 'plain' }], source })]
    expect(projectFilesForModel(messages)).toBe(messages)
  })

  it('projects direct and nested files to stable metadata plus durable recognized text', () => {
    const messages = [createUserMessage({
      content: [file('Quarterly revenue'), {
        type: 'tool-result',
        toolCallId: CallId('nested-file'),
        content: [file()],
      }],
      source,
    })]
    expect(projectFilesForModel(messages)[0]?.content).toEqual([
      {
        type: 'text',
        text: '[attached file: brief.pdf; application/pdf; 12 bytes; attachment sha256:bbbbbbbb]\nQuarterly revenue',
      },
      {
        type: 'tool-result',
        toolCallId: CallId('nested-file'),
  ...
  it('preserves a file only when the selected route can serialize its exact modality', () => {
    const messages = [createUserMessage({ content: [file()], source })]
    expect(projectFilesForModel(messages, ['text', 'pdf'])).toBe(messages)
    expect(projectFilesForModel(messages, ['text', 'image'])[0]?.content[0]?.type).toBe('text')
  })

  it('replaces oldest native files when their base64 request payload exceeds the route bound', () => {
    const messages = [createUserMessage({ content: [file(), file()], source })]
    expect(offloadRequestFiles(messages, 16)[0]?.content).toEqual([
      { type: 'text', text: '[attached file: brief.pdf; application/pdf; 12 bytes; attachment sha256:bbbbbbbb]' },
      file(),
    ])
  })
})
...
describe('projectImagesForTextModel', () => {
  ...
  it('replaces direct and nested images while retaining unaffected messages and blocks', () => {
    ...
    const visual = createUserMessage({
      content: [{ type: 'text', text: 'lead' }, image(3, 'recognized diagram'), unchangedNested, nested],
      source,
    })
    ...
    expect(projected[1]?.content).toEqual([
      ...
      {
        type: 'text',
        text: '[image represented as text because this model accepts text only; attachment sha256:aaaaaaaa]\nrecognized diagram',
      },
      unchangedNested,
      {
        ...nested,
        content: [
          ...
          { type: 'text', text: '[image represented as text because this model accepts text only; attachment sha256:aaaaaaaa]' },
