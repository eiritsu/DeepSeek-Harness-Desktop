...
可识别的 `owned_by` 值会选择对应 provider 声明。配置的完整 `baseURL` 若与唯一 provider API 精确匹配，也能为本地路由别名提供同一身份。两者都不存在时，容量取同 ID 声明的最小值，输入模态取共同交集，而推理等级要求声明完全一致。路由名、协议名、部分 URL 与模型名称模式均不构成能力证据。
...
- **不透明 owner 采用保守结果**：没有可识别的 `owned_by` 或精确提供方端点匹配时，容量取同 ID 声明的最小值，输入模态取共同交集；推理等级仍要求声明完全一致，并且绝不合并提供方特有内容。
