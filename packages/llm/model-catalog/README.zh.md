...
可识别的 `owned_by` 值会选择对应 provider 声明。配置的完整 `baseURL` 若与唯一 provider API 精确匹配，也能为本地路由别名提供同一身份。两者都不存在时，容量取同 ID 声明的最小值，输入模态与推理等级取共同交集，只保留每个精确同 ID 声明都支持的能力。路由名、协议名、部分 URL 与模型名称模式均不构成能力证据。
...
- **不透明 owner 采用保守结果**：没有可识别的 `owned_by` 或精确提供方端点匹配时，容量取同 ID 声明的最小值，输入模态与推理等级取共同交集，并且绝不合并提供方特有内容。
...
- **推理等级依赖上游声明**：`reasoning_options` 中类型为 `effort` 的标准等级会进入模型能力，`none` 与伴随 `effort` 的 `toggle` 会映射为规范的 `off`；上游没有等级列表时，插件不会为该模型猜测支持项。没有任何可选启用等级的单独 toggle 不会生成无法使用的单项选择器。
