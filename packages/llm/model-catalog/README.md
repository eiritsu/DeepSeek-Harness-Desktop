...
- **Opaque ownership is conservative** — without a recognized `owned_by` or exact provider endpoint match, capacities use the smallest same-ID declaration, while input modalities and reasoning levels use their intersections; provider-specific extras never combine.
...
- **Reasoning levels require an upstream declaration** — standard levels from `reasoning_options` entries of type `effort` become model capabilities, `none` and a companion `toggle` become the canonical `off` level, and the plugin does not guess levels when the source omits them. A toggle without any selectable enabled level does not create an unusable one-item selector.
