...
Profile Bundle that retrieves and persists complete model records from the live `models.dev` catalog, while projecting the input modalities, context/output capacities, and per-model reasoning levels that Harness consumes. Fields absent from the dynamic source continue through pi-ai's installed catalog. The shipped Web profile enables it by default; its patch mounts one Host plugin and changes no provider configuration.

The catalog marks its runtime enrichment as `authoritative`: upstream modality, context, output-capacity, and reasoning fields replace stale local values in model discovery and actual prepared calls. A field absent upstream remains local.

The plugin preserves the upstream model JSON in its durable snapshot, fills fields omitted from a discovery candidate, and registers exact-model input, capacity, and reasoning resolvers for runtime calls. Either lookup refreshes a stale snapshot; concurrent lookups share that refresh, success replaces the durable snapshot, and failure retains the last-good data. A recognized owner selects that provider's exact dynamic declaration, and the Models page preserves a discovered `owned_by` value on an adopted model. When the owner is a local alias, an exact configured `baseURL` match against one `models.dev` provider API supplies the same identity without per-model mappings. Model IDs are matched case-insensitively so casing aliases stay in one declaration set. Without an owner or endpoint match, capacities and reasoning require identical same-id declarations; input modalities use the intersection shared by all matching declarations, so optional PDF/audio differences do not erase a common image capability. An uncovered field falls back to the same pi-ai lookup. Endpoint-disclosed discovery fields and earlier enrichers remain authoritative, while runtime catalog capacities and reasoning levels replace stale installed or saved capability values without rewriting settings. Route names, protocols, partial URLs, and model-name patterns are never treated as capability evidence.
