...
export async function apply(ctx: Context, config: Config): Promise<void> {
  ...
  ctx.llm.registerModelMetadataEnricher('models.dev', async ({ provider, model, metadata, signal }) => {
    ...
    const patch = {
      ...
      // that declaration. Runtime input resolvers (including pi-ai) receive
  ...
  ctx.llm.registerModelReasoningResolver(async ({ model, ownedBy, baseURL, signal }) => {
    await catalog.refresh(signal)
    const metadata = catalog.metadata({ id: model, ...ownedBy === undefined ? {} : { ownedBy },
  ...
  ctx.llm.registerModelInputResolver(async ({ provider, model, ownedBy, baseURL, signal }) => {
    await catalog.refresh(signal)
    return catalog.metadata({ id: model, ownedBy: ownedBy ?? provider,
      ...baseURL === undefined ? {} : { baseURL } }).input
  })
  ctx.llm.registerModelCapacityResolver(async ({ provider, model, ownedBy, baseURL, signal }) => {
    await catalog.refresh(signal)
    const { contextWindow, maxOutputTokens } = catalog.metadata({ id: model, ownedBy: ownedBy ?? provider,
      ...baseURL === undefined ? {} : { baseURL } })
    return contextWindow === undefined && maxOutputTokens === undefined ? undefined : {
      ...contextWindow === undefined ? {} : { contextWindow },
      ...maxOutputTokens === undefined ? {} : { maxOutputTokens },
    }
  })
