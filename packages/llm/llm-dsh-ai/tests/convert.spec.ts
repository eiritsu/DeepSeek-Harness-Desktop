...
describe('toStreamChunks', () => {
  ...
  it('maps error events to error finish chunks (in-stream error style)', async () => {
    ...
    expect(chunks).toEqual([
      ...
      { type: 'finish', reason: { kind: 'error', failure: { message: 'boom', code: 'DSH_AI_ERROR' } } },
    ])
  })
  ...
  it('maps aborted error events to aborted finish', async () => {
    ...
    expect(chunks.at(-1)).toEqual({
      type: 'finish',
      reason: { kind: 'aborted', failure: { message: 'dsh-ai stream aborted', code: 'ABORTED' } },
    })
  })
...
describe('mapStopReason / mapUsage', () => {
  it.each([
    ...
    ['pending', {
      kind: 'error',
      failure: { message: 'dsh-ai stream for model "deepseek-v4-flash" ended pending', code: 'DSH_AI_ERROR' },
    }],
    ['deferred', {
      kind: 'error',
      failure: { message: 'dsh-ai deferred response for model "deepseek-v4-flash" is not supported', code: 'DSH_AI_ERROR' },
    }],
    ['aborted', { kind: 'aborted', failure: { message: 'dsh-ai stream aborted', code: 'ABORTED' } }],
  ...
  it('defaults the error message when pi-ai omits it', () => {
    expect(mapStopReason(assistant({ stopReason: 'error' })))
      .toEqual({ kind: 'error', failure: { message: 'dsh-ai stream error', code: 'DSH_AI_ERROR' } })
  })
  ...
  it('maps routable HTTP-ish error messages to stable codes', () => {
    ...
    }))).toMatchObject({ kind: 'error', failure: { code: 'DSH_AI_ERROR' } })
  })
  ...
  it.each([
    ...
    'upstream_stream_incomplete: Upstream response ended before completion',
  ...
  it('uses the resolved context window for silent and length-stop overflows', () => {
    ...
    expect(mapStopReason(silent, 100)).toEqual({
      kind: 'error',
      failure: {
        message: 'dsh-ai detected context overflow for model "deepseek-v4-flash"',
