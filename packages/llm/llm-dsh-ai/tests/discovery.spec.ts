...
import * as LlmDshAi from '@deepseek-ai/dsh-llm-dsh-ai'
...
describe('catalog-route model discovery', () => {
  it('answers from the installed registry, with capacities and no network call', async () => {
    ...
    const models = await ctx.llm.discoverModels('llm-dsh-ai', { provider: 'deepseek', baseURL: server.url })
  ...
  it('needs no endpoint for a route the catalog describes', async () => {
    const ctx = await harness()
    await expect(ctx.llm.discoverModels('llm-dsh-ai', { provider: 'deepseek' })).resolves.not.toHaveLength(0)
  })

  it('says where a route the catalog does not describe must get its models', async () => {
    const ctx = await harness()
    await expect(ctx.llm.discoverModels('llm-dsh-ai', { provider: 'acme-gateway' }))
    ...
    await expect(ctx.llm.discoverModels('llm-dsh-ai', { provider: 'acme-gateway', baseURL: '' }))
...
describe('draft-provider model discovery', () => {
  it('reads an OpenAI-compatible listing and keeps the capacities it discloses', async () => {
    ...
    const models = await ctx.llm.discoverModels('llm-dsh-ai', { baseURL: `${server.url}/v1`, apiKey: 'probe-key' })
  ...
  it('keeps a deployment path instead of resolving it away', async () => {
    ...
    await ctx.llm.discoverModels('llm-dsh-ai', { baseURL: `${server.url}/openai/v1/` })
  ...
  it('offers no credential when the draft names none', async () => {
    ...
    await ctx.llm.discoverModels('llm-dsh-ai', { baseURL: server.url })
  ...
  it('authenticates a configured route the draft cannot supply a key for', async () => {
    ...
    await ctx.llm.discoverModels('llm-dsh-ai', { provider: 'acme-gateway', baseURL: server.url })
    // A key typed into the form is the one being tested — possibly the
    // replacement for the stored one — so it wins.
    await ctx.llm.discoverModels('llm-dsh-ai', { provider: 'acme-gateway', baseURL: server.url, apiKey: 'typed' })
    // A route no profile declares yet is the create case: nothing is stored.
    await ctx.llm.discoverModels('llm-dsh-ai', { provider: 'not-declared-yet', baseURL: server.url })
  ...
  it('leaves a catalog route\'s credential unresolved, having never reached the network', async () => {
    ...
    await ctx.plugin(LlmDshAi, { providers: { deepseek: { apiKeyEnv: 'ABSENT_FOR_DISCOVERY' } } })
  ...
  it('drops unusable rows rather than failing the whole listing', async () => {
    ...
    expect(await ctx.llm.discoverModels('llm-dsh-ai', { baseURL: server.url }))
  ...
  it('points at the credential for a rejected one, and only then', async () => {
    ...
    for (const status of [401, 403]) {
      ...
      await expect(ctx.llm.discoverModels('llm-dsh-ai', { baseURL: refused.url, apiKey: 'wrong' }))
    ...
    await expect(ctx.llm.discoverModels('llm-dsh-ai', { baseURL: broken.url, apiKey: 'fine' }))
  ...
  it('reports a reply that is not a model listing', async () => {
    ...
    await expect(ctx.llm.discoverModels('llm-dsh-ai', { baseURL: server.url }))
      .rejects.toThrow(/no "data" array; enter this provider's models by hand/)

    const broken = await listingServer({ body: 'not json at all' })
    await expect(ctx.llm.discoverModels('llm-dsh-ai', { baseURL: broken.url }))
  ...
  it('refuses an oversized reply, whether its length is declared or streamed', async () => {
    ...
    await expect(ctx.llm.discoverModels('llm-dsh-ai', { baseURL: declared.url }))
    ...
    await expect(ctx.llm.discoverModels('llm-dsh-ai', { baseURL: streamed.url }))
      .rejects.toThrow(/answered with more than 4194304 bytes/)
  })
  ...
  it('reports an unreachable endpoint instead of an empty catalog', async () => {
    ...
    await expect(ctx.llm.discoverModels('llm-dsh-ai', { baseURL: 'http://127.0.0.1:9/v1' }))
  ...
  it.each(['anthropic-messages', 'azure-openai-responses', 'openai-codex-responses', 'google-generative-ai'])(
    ...
    async (api) => {
      ...
      const ctx = await harness()
      await expect(ctx.llm.discoverModels('llm-dsh-ai', { baseURL: 'https://gateway.example/v1', api }))
  ...
  it('reports cancellation during the body read as an abort, not a raw reason', async () => {
    ...
    const probe = ctx.llm.discoverModels('llm-dsh-ai', {
  ...
  it('honors caller cancellation', async () => {
    ...
    await expect(ctx.llm.discoverModels('llm-dsh-ai', {
  ...
  it('is offered for the namespace, and refuses one it does not serve', async () => {
    ...
    await expect(ctx.llm.discoverModels('llm-dsh-ai', { provider: 'openai' })).resolves.not.toHaveLength(0)
    await expect(ctx.llm.discoverModels('llm-deepseek', { baseURL: 'https://api.deepseek.com' }))
      .rejects.toMatchObject({ code: 'NO_DISCOVERY' })
    await expect(ctx.llm.discoverModels('llm-dsh-ai', { baseURL: '' }))
  ...
  it('withdraws the offer when the plugin unloads', async () => {
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    const fiber = await ctx.plugin(LlmDshAi, {})
    await expect(ctx.llm.discoverModels('llm-dsh-ai', { provider: 'openai' })).resolves.not.toHaveLength(0)

    await fiber.dispose()

    await expect(ctx.llm.discoverModels('llm-dsh-ai', { provider: 'openai' }))
      .rejects.toMatchObject({ code: 'NO_DISCOVERY' })
  })
})
