...
async function bootWithSettings(dir: string, config: LlmDshAi.Config): Promise<Context> {
  const ctx = new Context()
  await ctx.plugin(LlmRuntime)
  await ctx.plugin(FileSettingsProvider, { path: join(dir, 'settings.yaml'), watch: false })
  await ctx.plugin(LlmDshAi, config)
  return ctx
}

/** A complete hand-declared route: nothing about it exists in pi-ai's catalog. */
function gateway(baseURL: string, overrides: Record<string, unknown> = {}): LlmDshAi.Config {
  return {
    providers: {
      'acme-gateway': {
        apiKeyEnv: KEY_ENV,
...
async function harness(config: LlmDshAi.Config): Promise<Context> {
  const ctx = new Context()
  await ctx.plugin(LlmRuntime)
  await ctx.plugin(LlmDshAi, config)
  return ctx
...
describe('hand-declared providers', () => {
  ...
  it('rejects a model the route cannot identify', () => {
    const declare = (model: LlmDshAi.DshAiModelProfile): (() => unknown) =>
...
describe('per-model reasoning efforts', () => {
  ...
  function declared(models: LlmDshAi.DshAiModelProfile[]): Record<string, LlmDshAi.DshAiProviderProfile> {
  ...
  function modelOf(providers: Record<string, LlmDshAi.DshAiProviderProfile>, route = 'acme-gateway'): Model<Api> {
  ...
  it('rejects a declaration that offers nothing or spells a level it cannot send', () => {
    const declare = (efforts: NonNullable<LlmDshAi.DshAiModelProfile['reasoningEfforts']>): (() => unknown) =>
...
describe('compat switches', () => {
  ...
  function modelsOf(providers: Record<string, LlmDshAi.DshAiProviderProfile>, route: string): Map<string, Model<Api>> {
  ...
  it('refuses a valueless compat key written through the composed settings path', async () => {
    ...
    const dir = await home()
    const ctx = await bootWithSettings(dir, {})
    await expect(ctx.settings.update('llm-dsh-ai', {
      providers: {
        'acme-gateway': {
          api: 'openai-completions',
          baseURL: 'https://acme.test/v1',
...
describe('configurable-provider directory', () => {
  ...
  it('replaces its entries atomically as declared routes come and go', async () => {
    ...
    await ctx.settings.replace('llm-dsh-ai', {})
