...
import { DshAiAdapter } from '@deepseek-ai/dsh-llm-dsh-ai'
...
function adapterOf(
  providers: Record<string, LlmDshAi.DshAiProviderProfile>,
  apiKey: string | undefined = 'test-key',
): DshAiAdapter {
  return new DshAiAdapter({
...
describe('DshAiAdapter provider routing', () => {
  ...
  it('keeps prepared model metadata and dispatch on one profile snapshot', async () => {
    ...
    let providers: Record<string, LlmDshAi.DshAiProviderProfile> = {
      deepseek: { apiKeyEnv: 'PI_TEST_KEY', baseURL: first.url },
    }
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    ctx.llm.registerAdapter(['deepseek'], new DshAiAdapter({
      profiles: () => resolveProfiles(providers),
  ...
  it('uses the resolved catalog context window for usage-based overflow detection', async () => {
    ...
    expect(result.finish).toEqual({
      kind: 'error',
      failure: {
        message: `dsh-ai detected context overflow for model "${model.id}"`,
...
describe('provider profile lifecycle', () => {
  it('keeps adapter helpers off the package root', () => {
    for (const helper of [
      ...
      'mapUsage',
      'toStreamChunks',
    ]) expect(LlmDshAi).not.toHaveProperty(helper)
  })
  ...
  it('exposes the installed pi-ai model catalog through provider-neutral metadata', async () => {
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmDshAi, { providers: { openai: {} } })
  ...
  it('uses a supported profile reasoning value as the model default and rejects an unsupported one', async () => {
    ...
    await supported.plugin(LlmDshAi, {
    ...
    await unsupported.plugin(LlmDshAi, {
    ...
    await disabled.plugin(LlmDshAi, {
  ...
  it('uses authoritative catalog reasoning levels during dispatch', async () => {
    const server = await mockServer([{ events: textEvents }])
    const adapter = new DshAiAdapter({
      profiles: () => resolveProfiles({
        amd: {
          ...
          api: 'openai-completions',
          baseURL: `${server.url}/v1`,
          models: [{ id: 'Qwen3.8-Flash-Next', contextWindow: 262144, maxTokens: 131072 }],
        },
      }),
      resolveApiKey: () => Promise.resolve('test-key'),
      resolveReasoningEfforts: () => Promise.resolve(['low', 'medium', 'xhigh']),
      auth: memoryAuth(),
    })
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    ctx.llm.registerAdapter(['amd'], adapter)

    const rejected = await assemble(ctx, {
      provider: 'amd', model: 'Qwen3.8-Flash-Next', reasoningEffort: ReasoningEffortId('high'), messages: [],
    })
    expect(rejected.finish).toMatchObject({ kind: 'error', failure: { code: 'UNSUPPORTED_REASONING_EFFORT' } })
    expect(server.requests).toHaveLength(0)

    await assemble(ctx, {
      provider: 'amd', model: 'Qwen3.8-Flash-Next', reasoningEffort: ReasoningEffortId('xhigh'), messages: [],
    })
    expect(server.requests[0]).toMatchObject({ reasoning_effort: 'xhigh' })
  })
  ...
  it.each(['maxRetries', 'maxRetryDelayMs'] as const)(
    ...
    async (field) => {
      ...
      const ctx = new Context()
      await ctx.plugin(LlmRuntime)
      await expect(ctx.plugin(LlmDshAi, { providers: { openai: legacy } }))
  ...
  it('rejects invalid stream tunables at plugin load', async () => {
    ...
    for (const entry of invalid) {
      const ctx = new Context()
      await ctx.plugin(LlmRuntime)
      await expect(ctx.plugin(LlmDshAi, { providers: { openai: { ...entry } } }))
        .rejects.toThrow()
    }
  })
  ...
  it('rejects invalid nested retryPolicy at the provider-profile boundary', async () => {
    ...
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await expect(ctx.plugin(LlmDshAi, {
  ...
  it('rejects unsupported or unresolved image input before provider I/O', async () => {
    ...
    const drain = async (options: Parameters<DshAiAdapter['stream']>[0]): Promise<void> => {
