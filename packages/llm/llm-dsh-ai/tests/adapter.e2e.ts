...
import type { DshAiProviderProfile } from '@deepseek-ai/dsh-llm-dsh-ai'
...
async function harness(_model: string, config: Partial<DshAiProviderProfile> = {}) {
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY)('llm-dsh-ai e2e (real API)', () => {
  ...
  it('produces the same block structure as llm-deepseek for the same prompt', async () => {
    ...
    const [fromDeepSeek, fromDshAi] = await Promise.all([
    ...
    expect(blockKinds(fromDshAi)).toEqual(blockKinds(fromDeepSeek))
    expect(fromDshAi.finish.kind).toBe(fromDeepSeek.finish.kind)
  })
})
