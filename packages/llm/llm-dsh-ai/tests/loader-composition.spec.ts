 ...
 * settings-file, credentials-local, and a bare `llm-dsh-ai` row boot from a
...
/** Boot the dormant composition: a bare `llm-dsh-ai` row with no config at all. */
async function loadComposition(): Promise<{ ctx: Context; settingsPath: string }> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@deepseek-ai/dsh-llm-dsh-ai', LlmDshAi],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('llm-dsh-ai real dormant composition', () => {
