...
import type { DshAiReplayResponse } from '../src/replay.ts'
...
function expectNativeReplay(result: AssembledResult, profile: ProviderCase): DshAiReplayResponse {
  ...
  return (replayState as { response: DshAiReplayResponse }).response
}
...
for (const profile of providerCases) {
  describe.skipIf(profile.apiKey === undefined)(
    `llm-dsh-ai ${profile.provider} e2e (${profile.api})`,
    () => {
