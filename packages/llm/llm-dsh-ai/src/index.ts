/**
 * Generic dsh-ai LLM adapter plugin. One plugin instance owns a dict of
 * provider routes; a route naming an installed upstream provider inherits that
 ...
 * over the optional `llm-dsh-ai` user-settings section and the optional
 ...
 * ```
 *
 * @module @deepseek-ai/dsh-llm-dsh-ai
 */
...
import { DshAiAdapter } from './adapter.ts'
import { authContextFrom, credentialStoreFrom } from './auth.ts'
import { catalogProviderIds } from './catalog.ts'
import { assertServiceable, Config, resolveProfiles } from './config.ts'
import type { ResolvedDshAiProviderProfile } from './config.ts'
import { discoverModels } from './discovery.ts'
import { registerDshAiFlows } from './login.ts'

export { DshAiAdapter } from './adapter.ts'
export type { DshAiAdapterOptions } from './adapter.ts'
...
export type {
  ...
  DshAiModelOverride,
  DshAiModelProfile,
  DshAiProviderProfile,
  DshAiReasoningEfforts,
  DshAiThinkingFormat,
  ResolvedDshAiProviderProfile,
} from './config.ts'
export { recordKeyFor } from './auth.ts'
export { supportedProtocols } from './provider.ts'

export const name = 'llm-dsh-ai'
 ...
 */
function registrationFacts(profiles: ReadonlyMap<string, ResolvedDshAiProviderProfile>): unknown {
...
/** Register one generic dsh-ai adapter for all configured provider routes. */
export function apply(ctx: Context, config: Config): void {
  let current: () => Config = () => config
  let lastRaw: Config | undefined
  let memoized: ReadonlyMap<string, ResolvedDshAiProviderProfile> | undefined
  /**
   ...
   */
  const profiles = (): ReadonlyMap<string, ResolvedDshAiProviderProfile> => {
    const raw = current()
  ...
  const resolveApiKey = async (
    ...
    if (hit !== undefined && hit.length > 0) return assertUsableApiKey(hit, 'llm-dsh-ai', ref)
    throw new LlmError(
      `llm-dsh-ai: no credential for provider route "${provider}"; its profile resolves ${ref}, which is not`
  ...
  const adapter = new DshAiAdapter({
    profiles,
    resolveApiKey,
    ...
    onReplayDegrade: ({ provider, model, reason }) => {
      ctx.logger.warn(
        `llm-dsh-ai: unusable replay state on assistant history for route "${provider}/${model}";`
  ...
  ctx.inject(['authorization'], (authorized) => { registerDshAiFlows(authorized, auth) })
    ...
    settingsCtx.settings.installSection(ctx, NS, Config, config, {
      ...
      onChange: () => {
        ...
        try {
          ...
          ctx.logger.error('llm-dsh-ai: keeping the previously registered routes after a refused update')
        ...
        try {
          ...
          ctx.logger.error('llm-dsh-ai: keeping the previous configurable-provider directory after a refused update')
          ctx.logger.error(error)
        }
      },
    })
