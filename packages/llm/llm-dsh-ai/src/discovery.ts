 ...
 *
 * @module dsh-llm-dsh-ai/discovery
 */
...
export async function discoverModels(
  ...
  if (request.baseURL === undefined || request.baseURL.length === 0) {
    throw new LlmError(
      `dsh-ai ships no catalog for provider "${request.provider ?? ''}", so its models can only come from its`
  ...
  if (!LISTABLE_PROTOCOLS.has(api)) {
    throw new LlmError(
      `dsh-ai protocol "${api}" has no model listing this build can read; enter this provider's models by hand`,
