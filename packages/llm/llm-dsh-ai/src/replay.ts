...
export interface DshAiReplayResponse {
  kind: 'pi-ai'
  version: 2
  api: Api
  provider: string
...
interface DshAiReplayState {
  response: DshAiReplayResponse
  blocks: DshAiReplayBlock[]
}
...
export function toPiReplayState(message: AssistantMessage): ReplayEnvelope {
  const response: DshAiReplayResponse = {
    kind: 'pi-ai',
    version: 2,
    api: message.api,
  ...
  return {
    response,
    blocks: message.content.map((block): DshAiReplayBlock => {
      switch (block.type) {
        case 'text': return {
          type: 'text',
...
function readReplayState(value: unknown): DshAiReplayState {
  ...
  return {
    response: response as unknown as DshAiReplayResponse,
    blocks: blocks as DshAiReplayBlock[],
  }
}
