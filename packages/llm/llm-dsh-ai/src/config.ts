/**
 * Configuration schema and provider-profile validation for the dsh-ai adapter.
 ...
 */
export const DEFAULT_INPUT: readonly DshAiModality[] = ['text']
...
export interface ResolvedDshAiProviderProfile
  extends Omit<DshAiProviderProfile, 'apiKeyEnv' | 'retryPolicy' | 'models' | 'displayName'> {
...
const compatProfile: z<DshAiCompatProfile> = z.object({
...
) as unknown as z<DshAiReasoningEfforts>
...
const modelProfile: z<DshAiModelProfile> = z.object({
  id: z.string().required(),
  ...modelFields,
})

/** A {@link modelProfile} whose id lives in the `modelOverrides` dict key. */
const modelOverride: z<DshAiModelOverride> = z.object(modelFields)
...
function rejectRemovedFields(provider: string, source: DshAiProviderProfile): void {
  const legacy = source as DshAiProviderProfile & {
    provider?: unknown
    maxRetries?: unknown
    maxRetryDelayMs?: unknown
  }
  if ('provider' in legacy) {
    throw new Error(`llm-dsh-ai: provider "${provider}" sets "provider", which moved to the providers dict key`)
  }
  if ('maxRetries' in legacy || 'maxRetryDelayMs' in legacy) {
    throw new Error(
      `llm-dsh-ai: provider "${provider}" sets maxRetries or maxRetryDelayMs, which were removed;`
...
export function resolveProfiles(
  providers: Readonly<Record<string, DshAiProviderProfile>> | undefined,
): Map<string, ResolvedDshAiProviderProfile> {
  if (Array.isArray(providers)) {
    throw new Error('llm-dsh-ai: providers is now a dict keyed by provider route, not an array of profiles')
  }
  const entries = Object.entries(providers ?? {})
  const resolved = new Map<string, ResolvedDshAiProviderProfile>()
  for (const [provider, source] of entries) {
    rejectRemovedFields(provider, source)
    if (provider.length === 0) throw new Error('llm-dsh-ai: provider names must be non-empty')
    if (source.baseURL !== undefined && source.baseURL.length === 0) {
      throw new Error(`llm-dsh-ai: provider "${provider}" has an empty baseURL`)
    }
    if (source.displayName !== undefined && source.displayName.length === 0) {
      throw new Error(`llm-dsh-ai: provider "${provider}" has an empty displayName`)
    }
    ...
    if (!Number.isFinite(streamIdleTimeoutMs)
      ...
      throw new Error(
        `llm-dsh-ai: provider "${provider}" streamIdleTimeoutMs must be a positive finite number no greater than ${MAX_TIMER_DELAY_MS}`,
      )
    }
    ...
    if (!Number.isInteger(maxRequestImageBytes) || maxRequestImageBytes <= 0) {
      throw new Error(`llm-dsh-ai: provider "${provider}" maxRequestImageBytes must be a positive integer`)
    }
    ...
    if (!Number.isSafeInteger(requestImagePixelBudget) || requestImagePixelBudget <= 0) {
      throw new Error(`llm-dsh-ai: provider "${provider}" requestImagePixelBudget must be a positive safe integer`)
    }
    ...
    if (!Number.isSafeInteger(requestImageMaxBytes) || requestImageMaxBytes <= 0) {
      throw new Error(`llm-dsh-ai: provider "${provider}" requestImageMaxBytes must be a positive safe integer`)
    }
    ...
    if (defaultInput.length === 0) {
      throw new Error(`llm-dsh-ai: provider "${provider}" defaultInput must name at least one modality`)
    }
    ...
    resolved.set(provider, {
      ...
      retryPolicy: resolveRetryPolicy(retryPolicy, `llm-dsh-ai: provider "${provider}" retryPolicy`),
      ...
      externallyResolvableReasoningModels: catalog.externallyResolvableReasoningModels,
