...
import type { DshAiAuthInjection } from './adapter.ts'
 ...
 */
export function registerDshAiFlows(ctx: Context, auth: DshAiAuthInjection): void {
  for (const providerId of catalogProviderIds()) {
    ...
    if (!isCredentialKeySegment(providerId)) {
      ctx.logger.warn(
        'llm-dsh-ai: catalog provider "%s" cannot address a credential record; its sign-in is not offered',
        providerId)
      continue
    }
