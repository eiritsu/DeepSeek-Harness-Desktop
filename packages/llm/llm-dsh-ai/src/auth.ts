 ...
 */
export const RECORD_SCOPE = 'llm-dsh-ai'
...
function writableStore(ctx: Context): CredentialProvider {
  ...
  if (credentials === undefined) {
    throw new LlmError(
      'llm-dsh-ai: this composition mounts no credentials service, so there is nowhere to store the'
  ...
  return {
    ...
    async modify(providerId, mutate) {
      if (!isCredentialKeySegment(providerId)) {
        throw new LlmError(
          `llm-dsh-ai: provider id "${providerId}" cannot address a stored credential record (a record id is a`
