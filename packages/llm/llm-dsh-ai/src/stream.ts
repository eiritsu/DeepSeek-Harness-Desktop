...
function classifyDshAiError(message: string): string {
  ...
  if (/stream ended (?:before|without)\b|upstream[_ -]stream[_ -]incomplete\b/i.test(message)) return 'TRANSPORT'
...
export function mapStopReason(message: AssistantMessage, contextWindow?: number): FinishReason {
  ...
  if (piAiOverflow || harnessOverflow) {
    return {
      kind: 'error',
      failure: {
        message: message.errorMessage ?? `dsh-ai detected context overflow for model "${message.model}"`,
  ...
  switch (message.stopReason) {
    ...
    case 'pending': return {
      kind: 'error',
      failure: { message: `dsh-ai stream for model "${message.model}" ended pending`, code: 'DSH_AI_ERROR' },
    }
    case 'deferred': return {
      kind: 'error',
      failure: { message: `dsh-ai deferred response for model "${message.model}" is not supported`, code: 'DSH_AI_ERROR' },
    }
    case 'aborted': return {
      kind: 'aborted',
      failure: { message: message.errorMessage ?? 'dsh-ai stream aborted', code: 'ABORTED' },
    }
    case 'error': {
      const text = message.errorMessage ?? 'dsh-ai stream error'
      return { kind: 'error', failure: { message: text, code: classifyDshAiError(text) } }
    }
  }
}
...
export async function* toStreamChunks(
  ...
  throw new LlmError('dsh-ai event stream ended without done/error', 'STREAM_CLOSED')
}
