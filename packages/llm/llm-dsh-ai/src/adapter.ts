/**
 * Generic dsh-ai implementation of the Harness LLM seam.
 *
...
import type { ResolvedDshAiProviderProfile } from './config.ts'
import { toPiContext } from './context.ts'
import { toStreamChunks } from './stream.ts'

/** One resolution's frozen view: the profiles and the collection built from them. */
interface DshAiSnapshot {
  /** The resolved profiles this collection was built from, used as its identity. */
  profiles: ReadonlyMap<string, ResolvedDshAiProviderProfile>
  /** Providers for exactly those profiles; never mutated once published. */
  models: Models
}

interface ResolvedDshAiModel {
  readonly model: Model<Api>
  readonly inputModalities: readonly ModelModality[]
}

/** Constructor options for {@link DshAiAdapter}: the two resolution hooks the plugin owns. */
export interface DshAiAdapterOptions {
   ...
   */
  resolveApiKey: (provider: string, profile: ResolvedDshAiProviderProfile) => Promise<string | undefined>
...
export interface DshAiAuthInjection {
...
function profileOptions(
  profile: ResolvedDshAiProviderProfile,
...
function resolveReasoningLevel(
  ...
  throw new LlmError(
    `dsh-ai provider "${model.provider}" model "${model.id}" does not support reasoning effort "${effort}"`,
...
/** Apply exact external catalog levels to the pi-ai dispatch descriptor. */
...
/**
 * dsh-ai multi-provider adapter. Each operation reads the current
 * profiles, so a configuration change reaches the next request without a
 * restart; model descriptors come from the collection those profiles built.
 */
export class DshAiAdapter extends LlmAdapter {
  private snapshot: DshAiSnapshot | undefined

  constructor(private readonly config: DshAiAdapterOptions) {
    super()
  }
  ...
  private profileOf(snapshot: DshAiSnapshot, provider: string): ResolvedDshAiProviderProfile {
    const profile = snapshot.profiles.get(provider)
    if (profile === undefined) {
      throw new LlmError(`dsh-ai adapter does not own provider "${provider}"`, 'NO_ADAPTER')
    }
    return profile
  }

  /** The configured descriptor for one exact route/model pair within one snapshot. */
  private modelOf(snapshot: DshAiSnapshot, provider: string, model: string): Model<Api> {
    this.profileOf(snapshot, provider)
    const resolved = snapshot.models.getModel(provider, model)
    if (resolved === undefined) {
      throw new LlmError(`dsh-ai provider "${provider}" has no configured model "${model}"`, 'UNKNOWN_MODEL')
    }
    return resolved
  }
  ...
  private async modelForCall(
    snapshot: DshAiSnapshot,
    provider: string,
    model: string,
    signal?: AbortSignal,
  ): Promise<ResolvedDshAiModel> {
  ...
  private async * streamWithSnapshot(
    ...
    snapshot: DshAiSnapshot,
    preparedModel?: ResolvedDshAiModel,
  ): AsyncIterable<StreamChunk> {
    if (options.stop !== undefined) {
      throw new LlmError('llm-dsh-ai does not support GenerateOptions.stop', 'UNSUPPORTED_OPTION')
    }
    ...
    try {
      ...
      if (containsImage && !resolved.inputModalities.includes('image')) {
        throw new LlmError(`dsh-ai model "${model.id}" does not support image input`, 'UNSUPPORTED_CONTENT')
      }
      const attachments = containsImage ? this.config.resolveAttachments?.() : undefined
      if (containsImage && attachments === undefined) {
        throw new LlmError('dsh-ai image input requires the durable attachment service', 'UNSUPPORTED_CONTENT')
      }
      ...
      if (timeoutOf(watchdog.signal, 'LLM_STREAM_IDLE_TIMEOUT') !== undefined) {
        throw new LlmError(`dsh-ai stream idle timeout after ${streamIdleTimeoutMs}ms`, 'TIMEOUT', { cause: error })
      }
      if (options.signal?.aborted) {
        throw new LlmError('dsh-ai request aborted by caller', 'ABORTED', { cause: error })
      }
      throw error
    } finally {
      consumer.abort('pi-ai stream consumer stopped')
