 ...
 */
const MODALITY_GATE: Record<DshAiModality, true> = {
  text: true,
  image: true,
}

/** Every request modality a profile may declare. */
export const MODALITIES = Object.keys(MODALITY_GATE) as readonly DshAiModality[]
 ...
 */
function declaredInput(configured: readonly DshAiModality[] | undefined): Model<Api>['input'] | undefined {
 ...
 * {@link DshAiCompatProfile.chatTemplateKwargs} carries their kwargs;
 * `baseten` is nameable because {@link DshAiCompatProfile.chatTemplateArgs}
 * carries its arguments.
 */
const THINKING_FORMAT_GATE: Record<DshAiThinkingFormat, true> = {
  'openai': true,
  'deepseek': true,
  'openrouter': true,
  'together': true,
...
export const SUPPORTED_THINKING_FORMATS = Object.keys(THINKING_FORMAT_GATE) as readonly DshAiThinkingFormat[]

/** The output-cap field spellings pi-ai accepts. */
export type DshAiMaxTokensField = NonNullable<OpenAICompletionsCompat['maxTokensField']>

/** Drift gate over {@link DshAiMaxTokensField}; an upstream spelling added here fails compilation until named. */
const MAX_TOKENS_FIELD_GATE: Record<DshAiMaxTokensField, true> = {
  max_completion_tokens: true,
  max_tokens: true,
}

/** The output-cap field spellings a profile may name. */
export const MAX_TOKENS_FIELDS = Object.keys(MAX_TOKENS_FIELD_GATE) as readonly DshAiMaxTokensField[]

/** The prompt-cache marker conventions pi-ai accepts. */
export type DshAiCacheControlFormat = NonNullable<OpenAICompletionsCompat['cacheControlFormat']>

/** Drift gate over {@link DshAiCacheControlFormat}; a new upstream convention fails compilation until named. */
const CACHE_CONTROL_FORMAT_GATE: Record<DshAiCacheControlFormat, true> = {
  anthropic: true,
}

/** The prompt-cache marker conventions a profile may name. */
export const CACHE_CONTROL_FORMATS = Object.keys(CACHE_CONTROL_FORMAT_GATE) as readonly DshAiCacheControlFormat[]

/** The request-state placeholders a `chat_template_kwargs` value may name. */
export type DshAiChatTemplateVar = Extract<ChatTemplateKwargValue, { $var: string }>['$var']

/** Drift gate over {@link DshAiChatTemplateVar}; a new upstream placeholder fails compilation until named. */
const CHAT_TEMPLATE_VAR_GATE: Record<DshAiChatTemplateVar, true> = {
  'thinking.enabled': true,
  'thinking.effort': true,
}

/** The request-state placeholders a profile may name. */
export const CHAT_TEMPLATE_VARS = Object.keys(CHAT_TEMPLATE_VAR_GATE) as readonly DshAiChatTemplateVar[]
 ...
 */
export type EveryProfileFieldIsOffered = AssertNever<Exclude<keyof DshAiCompatProfile, OfferedCompatField>>
 ...
 */
export type EveryOfferedFieldIsDocumented = AssertNever<Exclude<OfferedCompatField, keyof DshAiCompatProfile>>
...
export type EveryProfileFieldMatchesUpstream = AssertTrue<
  DshAiCompatProfile extends Partial<Pick<UpstreamCompat, OfferedCompatField>>
    ? Partial<Pick<UpstreamCompat, OfferedCompatField>> extends DshAiCompatProfile ? true : false
    : false
>
 ...
 */
function configuredCompatEntries(compat: DshAiCompatProfile | undefined): readonly (readonly [string, unknown])[] {
...
function assertOfferedCompatFields(
  provider: string,
  site: string,
  compat: DshAiCompatProfile | undefined,
): void {
...
export interface RouteCatalogRequest {
  ...
  models?: readonly DshAiModelProfile[]
  /** Installed-catalog customizations by model id; only meaningful while `models` is absent. */
  modelOverrides?: Readonly<Record<string, DshAiModelOverride>>
...
function invalid(provider: string, detail: string): never {
  throw new Error(`llm-dsh-ai: provider "${provider}" ${detail}`)
}
...
function resolveModelCompat(
  provider: string,
  entry: DshAiModelProfile,
  route: DshAiCompatProfile | undefined,
  base: Model<Api> | undefined,
  api: string,
...
export interface RouteCatalog {
  ...
  inputModalities: ReadonlyMap<string, readonly DshAiModality[]>
  /** Upstream owners preserved from configuration, keyed by model id. */
  modelOwners: ReadonlyMap<string, string>
  /** Model ids whose configuration does not pin input modalities. */
  externallyResolvableInputModels: ReadonlySet<string>
  /** Model ids whose configuration does not pin reasoning capabilities. */
...
export function resolveRouteModels(request: RouteCatalogRequest): RouteCatalog {
  ...
  const entries: readonly DshAiModelProfile[] = configured.length > 0
    ? configured
  ...
  const inputModalities = new Map<string, readonly DshAiModality[]>()
