...
class StaticAttachmentStore extends AttachmentStore {
  ...
  saveFile(): Promise<never> { return Promise.reject(new Error('dynamic-config fixture is image-only')) }
  readFile(): Promise<never> { return Promise.reject(new Error('dynamic-config fixture is image-only')) }
