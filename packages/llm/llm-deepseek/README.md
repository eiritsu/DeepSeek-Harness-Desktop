...
The core exposes the same `Default`, `Off`, `Low`, `High`, and `Max` controls for every model. Omitting `reasoningEffort` leaves this profile's deployment default in effect, which is `high` unless configured otherwise. `agent/request` can replace the value on each conversation step; an explicit value is logged in `request/header`. `low`, `high`, and `max` enable thinking and serialize as the same official top-level `reasoning_effort` value; standard `off` instead serializes `thinking.type: disabled` and omits `reasoning_effort`. A deployment policy that cannot honor a selected value fails with `UNSUPPORTED_REASONING_EFFORT` before network I/O.

`thinking: disabled` is a deployment lock whose default is `off`. The core selector still shows all five standard choices; omitting `reasoningEffort` or selecting `off` is valid, while a direct request for `low`, `high`, or `max` fails before network I/O. Configuring one of those enabled levels as the deployment default fails plugin loading. A request with `GenerateOptions.purpose: 'session-title'` also forces thinking disabled and omits the selected effort, reserving its bounded output for visible title text without changing conversation or compaction defaults.
...
- Standard `off` maps to `thinking: {type: 'disabled'}` and never crosses the wire as `reasoning_effort: 'off'`.
