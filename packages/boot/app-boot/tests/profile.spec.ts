...
describe('loadProfile', () => {
  ...
  it('normalizes only exact installation-owned bundle tuples', () => {
    const anchor = stageInstallation({
      ...
      '@deepseek-ai/dsh-file-recognizer-office': { patch: '[]\n' },
      '@deepseek-ai/dsh-model-catalog': { patch: '[]\n' },
      'custom-bundle': { patch: '[]\n' },
    })
    const home = tmp()
    ...
    const extendedWebHome = tmp()
    const extendedWeb = resolveProfileDir('web', extendedWebHome)
    initProfile(extendedWeb, [
      '@deepseek-ai/dsh-base', '@deepseek-ai/dsh-web-app',
      '@deepseek-ai/dsh-file-recognizer-office', 'custom-bundle',
    ])
    loadProfile('t', 'web', anchor, extendedWebHome)
    expect(readProfileManifest('t', extendedWeb).dsh?.profile?.bundles).toEqual([
      '@deepseek-ai/dsh-base', '@deepseek-ai/dsh-web-app',
      '@deepseek-ai/dsh-file-recognizer-office',
      '@deepseek-ai/dsh-model-catalog', 'custom-bundle',
    ])
