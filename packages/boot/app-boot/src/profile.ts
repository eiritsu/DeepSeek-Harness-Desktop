...
function normalizeShippedProfile(name: string, dir: string, manifest: ProfileManifest): ProfileManifest {
  ...
  if (installationOwned === undefined || current === undefined || bundles === undefined) return manifest
  const exactInstallationTuple = sameBundles(bundles, installationOwned)
  const webWithInstalledBundles = name === 'web'
    && bundles.length >= installationOwned.length
    && installationOwned.every((bundle, index) => bundles[index] === bundle)
    && WEB_DEFAULT_BUNDLES.some(bundle => !bundles.includes(bundle))
  if (!exactInstallationTuple && !webWithInstalledBundles) return manifest
  const normalizedBundles = webWithInstalledBundles
    ? [
      ...bundles.slice(0, installationOwned.length),
      ...WEB_DEFAULT_BUNDLES,
      ...bundles.slice(installationOwned.length).filter(bundle => !WEB_DEFAULT_BUNDLES.includes(
        bundle as (typeof WEB_DEFAULT_BUNDLES)[number],
      )),
    ]
    : [...current]
  const normalized: ProfileManifest = {
    ...
    dsh: {
      ...manifest.dsh,
      profile: { ...manifest.dsh?.profile, bundles: normalizedBundles },
    },
  }
