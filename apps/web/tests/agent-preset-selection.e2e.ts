...
// session starts. Before that, the new-session chip and the default setting
// can both select the composition. After it, the session header names what
// the session runs and offers no control at all, because the host answers
...
describe('web e2e: agent-preset selection', () => {
  ...
  it('applies a changed default to the current blank new session', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-agent-preset-default'))
    await page.getByRole('button', { name: 'Settings', exact: true }).click()
    const settings = page.getByRole('dialog', { name: 'Settings' })
    await settings.waitFor({ timeout: 10_000 })
    await settings.getByRole('button', { name: 'Agent presets' }).click()
    await settings.getByRole('button', { name: 'Set as default: PTC mode' }).click()
    await settings.getByRole('button', { name: 'Close' }).click()

    await page.getByRole('button', { name: 'PTC mode' }).waitFor({ timeout: 10_000 })
    await expect.poll(() => livePreset(scaffold.baseUrl), { timeout: 15_000 }).toBe('code')

    // Restore the lane's declared default so later cases keep their own
    // starting condition rather than depending on this assertion's mutation.
    await page.getByRole('button', { name: 'Settings', exact: true }).click()
    await settings.getByRole('button', { name: 'Agent presets' }).click()
    await settings.getByRole('button', { name: 'Set as default: Standard mode' }).click()
    await settings.getByRole('button', { name: 'Close' }).click()
