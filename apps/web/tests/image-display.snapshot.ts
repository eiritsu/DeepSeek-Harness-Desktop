...
// intake chain (Add launcher → files/folders picker → paste → ordered attachment rail → remove).
...
it('exposes Files and folders and accepts pasted images and generic files', async () => {
  mountAssembledApp()
  ...
  fireEvent.click(add)
  const filesAndFolders = await screen.findByText('Files and folders')
  const pickerClick = vi.spyOn(HTMLInputElement.prototype, 'click').mockImplementation(() => undefined)
  fireEvent.mouseDown(filesAndFolders)
  ...
  // Non-image formats take the generic file path instead of being rejected by
  // the image allowlist.
  fireEvent.paste(textarea, {
    clipboardData: {
  ...
  expect(screen.queryByText('Only PNG, JPG, WebP, and GIF images are supported')).toBeNull()
  fireEvent.keyDown(textarea, { key: 'Enter' })
  await waitFor(() => {
    expect(document.querySelector('[role="group"][aria-label="Pending attachments"]')).toBeNull()
    const historyFile = [...document.querySelectorAll<HTMLElement>('[data-align="end"] button')]
      .find(button => button.textContent?.includes('notes.txt') ?? false)
    expect(historyFile?.textContent).toContain('text/plain · 1 B')
  }, { timeout: 10_000 })
})
...
it('accepts a whole-page drop under the limits-labeled overlay and refuses an over-limit batch at intake', async () => {
  ...
  expect(overlay.textContent).toContain('Drag files or folders here to add them')
  await waitFor(() => {
    expect(overlay.textContent).toContain('Images: 20 up to 5MB each; files: 20 up to 32MB each')
  })
