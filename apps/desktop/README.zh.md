...
本地启动页提供启动状态和可用恢复操作。加载后的 dsh 渲染进程接收桌面协议标记及范围受限的数据、Skill 和插件库 bridge。插件库与 Lite 使用同一个共享覆盖层，只接受结构化的发现、审查、安装、移除和审计操作；它无法访问通用文件系统、原始 Electron IPC、shell 或任意 pnpm 参数。
