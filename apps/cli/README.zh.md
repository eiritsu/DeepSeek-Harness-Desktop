...
`dsh.profile.bundles` 中列出的组合包先从 dsh 安装目录解析（`@deepseek-ai/dsh-base`、`@deepseek-ai/dsh-web-app`、`@deepseek-ai/dsh-headless`、`@deepseek-ai/dsh-file-recognizer-office`、`@deepseek-ai/dsh-model-catalog`），再从 profile 自身的 `node_modules` 解析；pnpm 会将树外插件安装到该目录。Web 模板默认启用 `Deepseek-Files` 与模型能力目录。
