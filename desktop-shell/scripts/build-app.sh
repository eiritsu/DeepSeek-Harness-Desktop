...
if [ "$DISTRIBUTION" = true ]; then
  ...
  NATIVE_ENTRY_LIB="$SOURCE_ROOT/native/system/packages/entry/lib"
  ...
  mkdir -p "$SNAPSHOT_ROOT/native/system/packages/entry"
  COPYFILE_DISABLE=1 /bin/cp -R "$NATIVE_ENTRY_LIB" "$SNAPSHOT_ROOT/native/system/packages/entry/"
