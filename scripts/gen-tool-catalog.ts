...
class CatalogAttachmentStore extends AttachmentStore {
  ...
  override saveFile(): Promise<never> {
    return Promise.reject(new Error('gen-tool-catalog: generic file writes are unreachable during schema harvest'))
  }

  override readFile(): Promise<never> {
    return Promise.reject(new Error('gen-tool-catalog: generic file reads are unreachable during schema harvest'))
  }
