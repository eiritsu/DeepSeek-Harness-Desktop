...
class TestAttachmentStore extends AttachmentStore {
  ...
  saveFile(): Promise<never> {
    return Promise.reject(new Error('test invariant attachment store does not save generic files'))
  }

  readFile(): Promise<never> {
    return Promise.reject(new Error('test invariant attachment store does not read generic files'))
  }
