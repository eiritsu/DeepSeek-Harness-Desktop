...
仓库为 pi-ai 0.85.1 的 OpenAI Responses 解析器保留了一个窄范围补丁：函数调用的 `arguments.done` 事件缺少或携带非字符串 `arguments` 时，改用已经累积的部分 JSON，而不是解引用该值。DSH 适配器还会保留已收到的参数分片，只有没有可用分片时才输出 `{}`，因此格式异常的网关完成事件不会再以不可序列化的分片使 Assistant 流结算失败。PTC 函数体运行时仍禁止静态 `import`/`export`；这条源码求值规则与提供方流归一化无关。
