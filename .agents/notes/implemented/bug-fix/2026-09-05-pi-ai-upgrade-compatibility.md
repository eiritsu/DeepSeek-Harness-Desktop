...
The repository carries a narrow patch for pi-ai 0.85.1's OpenAI Responses parser: a function-call `arguments.done` event with a missing or non-string `arguments` value reuses the accumulated partial JSON instead of dereferencing it. The DSH adapter also retains streamed argument fragments and emits `{}` only when no usable fragment exists, so malformed gateway completions cannot make durable Assistant stream settlement fail with a non-serializable chunk. Static `import`/`export` remains forbidden in the PTC function-body runtime; that source-evaluation rule is unrelated to provider stream normalization.
