...
这种拆分还掩盖了其他偏差。preset 挂载了一次性 Bash，而不是 RL harness 使用的[持久 Bash](../feature/2026-07-29-persistent-bash-str-replace-editor.zh.md)，并且遗漏了 RL 压缩（compaction）策略。保留两个所有者，会使今后每次修改提示词和工具时都必须验证二者的交叉组合；Web 压缩由宿主 profile 所有。

## 决策

随附的 Web `minimal` preset 是 RL agent 组合在 Web 中的唯一所有者。它声明 entry 本地的 PTY 注册表与本地后端、带 RL 环境描述且超时为 300 秒的持久 `bash`，以及 `str_replace_editor`。工具呈现仍由部署选择。后续的[裸双工具运行时决策](../feature/2026-08-11-minimal-profiles-bare-two-tool-runtime.zh.md)取代了本记录最初的文件系统提供方选择：当前 preset 挂载 entry 本地的 `fs-local` 提供方，Web 压缩则来自宿主 profile。编辑器不接受 `requireAbsolutePath` 设置，因为要求绝对路径是它的无条件约定。
