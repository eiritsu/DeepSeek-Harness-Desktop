...
- `LlmDiscoveredModel` makes every field but `id` optional, because most listings disclose an id and nothing else. Explicit input modalities remain candidate metadata through the Service Definition and RPC, so an adopting surface stores them on that model without another user declaration. Absence stays unknown. The reply is candidates, not a catalog: a surface adopting one still owes metadata the adapter requires but the provider did not disclose.
...
Metadata that an endpoint omits can be supplied by the ordered, fill-only plugins defined in the [model-discovery enrichment decision](2026-08-24-model-discovery-enrichment-plugins.md). Interrogation remains the only source of model ids and explicit endpoint facts; enrichment neither adds models nor rewrites them.

A person adding a gateway can ask it what it serves instead of hunting through its documentation, and the answer arrives as candidates they choose from rather than as configuration written behind their back. Adopting a candidate automatically stores capacities and input modalities the provider disclosed; configuration remains necessary only for facts its listing omits or for correcting a gateway whose claim differs from its behavior. The seam gained a registry that is deliberately small: one offer per namespace, no storage, no lifecycle beyond the fiber.
...
`packages/llm/llm/tests/topology.spec.ts` covers the registry: one offer per namespace, disposal with the fiber, normalization that drops duplicate and unusable ids while preserving explicit modalities, and the `NO_DISCOVERY`/`INVALID_DISCOVERY` refusals. `packages/llm/llm-pi-ai/tests/discovery.spec.ts` drives the probe against local HTTP servers — disclosed capacities and modalities, unsupported modality values remaining unknown, a preserved deployment path, credential paths, catalog metadata without network access, malformed replies, cancellation, unsupported protocols, and both size-ceiling paths. `packages/host/apiproxy/tests/api-proxy-config.spec.ts` and the Models-page component tests preserve modalities across the RPC and candidate adoption without writing credentials.
