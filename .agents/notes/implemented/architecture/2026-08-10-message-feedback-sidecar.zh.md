...
只按 `SessionId` 建索引的伴随记录可能在该 id 以不同 header 身份重建后，继续存活于其所描述的日志生命周期之外。Session 级 revision 还会让无关消息的编辑彼此冲突，而普通 storage-domain 读／写不提供跨进程 compare-and-swap。Session disposal 只是从 live store 脱离，并非持久删除；真实清理因此监听持久化 seam 的显式删除事件。
...
服务不执行基于 detach 的伪删除级联。`session/disposed` 与 `host/session-removed` 表示脱离 live ownership，而非持久删除。权威 `session-persistence/deleted` 事件会把该 Session 的 sidecar 行删除排在已经接纳的反馈变更之后。带外移除日志后，伴随记录仍可能继续存在；不同的 `{createdAt, cwd}` 可阻止此类遗留记录变成后来复用该 id 的 Session 反馈。
...
**在 Session disposal 时删除反馈。** 不予采纳，因为 disposal 包含普通 detach 与 rollback 路径。把它当成持久删除会在 Session 日志仍存在时丢失反馈；清理只监听真正的 Session 删除权威。
