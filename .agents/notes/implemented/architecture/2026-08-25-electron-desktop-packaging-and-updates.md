...
One Desktop release number identifies the Electron artifact and its exact `@deepseek-ai/dsh` and `@deepseek-ai/dsh-desktop-host` dependencies. A release cannot select a different core version at build or runtime. Updating dsh therefore requires a new Electron release even when shell code is unchanged. The Electron application and artifacts use the `DeepSeek Harness Electron` product name; its local test identifier is `ai.deepseek.harness.desktop.electron`, keeping launch registration and installation separate from both Swift applications.
...
An explicit local macOS test invocation ad-hoc signs the embedded Mach-O files and Electron application without hardened runtime, strips ambient certificate and notarization credentials, isolates its output under `local-artifacts`, omits updater configuration and release completion metadata, and never enters the upload path. Its payload smoke exercises the packaged POSIX flock binding rather than the removed `fs-ext` dependency. It exercises the complete packaged runtime on the build host but cannot qualify as a distributable release.
...
The macOS arm64 local command passes the same target and bundled resources through preparation, applies only ad-hoc signatures, and emits a DMG, ZIP, or runnable directory outside the release artifact directory.
...
- Apple Silicon development hosts can exercise the packaged application without release credentials, while local artifacts remain ineligible for upload.
