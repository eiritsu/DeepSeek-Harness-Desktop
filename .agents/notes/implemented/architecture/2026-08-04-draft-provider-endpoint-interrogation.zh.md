...
- `LlmDiscoveredModel` 除 `id` 外每个字段都可选，因为大多数列表只公布 id。明确给出的输入模态会作为候选元数据贯穿 Service Definition 与 RPC，因此采纳界面会自动存入对应模型，无需用户再次声明；字段缺失则保持未知。回复是候选而非 catalog：采纳界面仍要补上适配器所需、但提供方没有公布的元数据。
...
`dsh-llm-pi-ai` 的实现只是一次朴素的 `GET {baseURL}/models`，且仅限 OpenAI 兼容协议。它们的列表形状是网关、自建服务与官方端点三方一致认可的那一种，而这正是该动作存在的场景。其余协议一律以 `DISCOVERY_UNSUPPORTED` 回答，让界面回退到手工填写，而不是把猜错的响应形状报成一个空提供方。解析器保留顶层或 `architecture.input_modalities` 中明确给出的受支持值（`text`、`image`），字段缺失或只含不支持值时保持未知；它绝不会根据模型 id 或所选 wire protocol 猜测能力。由 pi-ai 已安装 catalog 服务的路由无需联网，直接携带该 catalog 的确切模态。`baseURL` 按前缀而非待解析 URL 处理，因此 `https://gateway.example/openai/v1` 这类部署路径会保留其路径段。回复在四兆字节上限下读取，且上限落在实际收到的字节上——端点是用户自己填的 URL，因此会先看声明的 `content-length` 作为善意提示，但绝不把它当作边界；这与 `dsh-web-fetch` 面对自己的调用方提供 URL 时所用的两段式形状一致。
...
端点省略的元数据可以由[模型发现 enrichment 决策](2026-08-24-model-discovery-enrichment-plugins.zh.md)定义的有序、只补缺插件提供。询问仍是模型 id 与端点明确事实的唯一来源；enrichment 既不增加模型，也不改写它们。

接入网关的人可以直接问它服务什么，而不必去翻它的文档；答案以候选形式抵达，由用户自己挑选，而不是被背着写进配置。采纳候选会自动存储提供方公布的容量与输入模态；只有列表省略的事实，或网关声明与实际行为不一致时的修正，才需要配置。seam 因此多了一个刻意保持很小的注册表：每个 namespace 一份、不存储、生命周期不超出 fiber。
...
`packages/llm/llm/tests/topology.spec.ts` 覆盖注册表：每个 namespace 一份、随 fiber dispose（资源释放）、丢弃重复与不可用 id 且保留明确模态的归一化，以及 `NO_DISCOVERY`/`INVALID_DISCOVERY` 两种拒绝。`packages/llm/llm-pi-ai/tests/discovery.spec.ts` 针对本地 HTTP 服务器驱动探测——包括公布的容量与模态、不支持的模态值保持未知、部署路径保留、凭据路径、catalog 元数据无需联网、畸形响应、取消、不支持的协议，以及两种尺寸上限路径。`packages/host/apiproxy/tests/api-proxy-config.spec.ts` 与模型页组件测试共同覆盖模态贯穿 RPC 和候选采纳，且不会写入凭据。
