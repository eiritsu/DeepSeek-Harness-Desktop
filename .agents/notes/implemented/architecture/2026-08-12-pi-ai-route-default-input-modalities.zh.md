...
网关返回 `owned_by` 时，[模型发现 enrichment 插件](2026-08-24-model-discovery-enrichment-plugins.zh.md)可以在采纳候选时提供精确的已安装 catalog 声明；对于已有行和缺少可靠 ownership 的端点，这里的逐模型或逐路由手工声明仍是回退方法。
