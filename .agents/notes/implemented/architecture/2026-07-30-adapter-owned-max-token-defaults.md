...
`LlmResolvedModelInfo.defaultMaxTokens` carries an optional adapter-configured per-request output cap for one exact provider/model route. `LlmRuntime` validates it as a positive safe integer and materializes it into `LlmCallConfig.maxTokens` only when the caller omitted a value. A prepared call identifies the materialized `maxTokens` field as an adapter default; explicit request or Agent options remain unmarked and therefore win without clamping. Reasoning effort uses the core-standard controls and is never materialized as an adapter default.
