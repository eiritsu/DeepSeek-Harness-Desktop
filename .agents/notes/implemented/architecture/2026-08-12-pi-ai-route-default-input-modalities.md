...
The [model-discovery enrichment plugin](2026-08-24-model-discovery-enrichment-plugins.md) can supply an exact installed-catalog declaration during candidate adoption when a gateway returns `owned_by`; this manual per-model or route declaration remains the fallback for existing rows and endpoints without reliable ownership.
