...
- session 行 … 菜单：Rename / Fork session / Copy session ID / Archive session / Delete session。空白行也保留复制、归档和删除；rename 与 fork 需要首条 prompt 已实体化。workspace 组头 … 菜单的 Rename / Delete workspace 均已接线。菜单鼠标移出即关。
