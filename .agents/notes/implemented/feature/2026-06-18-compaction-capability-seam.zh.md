...
基础后端将摘要包装为既定的检查点上下文，并标记以便下一轮增量合并。原始摘要保留在 `compaction/summary` 上。框架是后端策略；seam 承诺由一条替换 user 消息承载可能带框架的摘要，并使用规范的检查点来源。检查点指令是领域中立的：它记录概念与实体、决定与约束、环境与资源、文件与产物、证据与验证、承诺与待办、风险和下一步，适用于开发、办公、运维、研究、规划和业务工作。环境事实按事实保留，密钥只保留名称和用途，不保留值。
