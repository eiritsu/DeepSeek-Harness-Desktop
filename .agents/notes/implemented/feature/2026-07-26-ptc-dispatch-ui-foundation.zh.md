...
1. **`run_code` 新增可选的 `description` 参数**（与 bash 相同的展示约定：主动语态、5-10 个词、展示在 UI 中；明确传入的空白值在执行时被拒绝）。`presentCall` 现在以该 description 或固定的 `Run code` 回退标签作为卡片标题，并把程序文本移入 `rawInput`。提示词侧的成本是每次调用多出几个 token；换来的是每个界面——TUI 卡片、ACP（Agent Client Protocol）标题、Web 行——都无需解析 TypeScript 就能获得可供人阅读的标签。
