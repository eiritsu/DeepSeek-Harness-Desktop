...
在 `'ptc'` 和 `'both'` 下，注册表拥有 `run_code` 作为保留的呈现传输通道，带一个必需参数和一个可选展示参数 `{ code: string; description?: string }`（description 为 UI 标注该调用，省略时使用固定回退标签）。它由一个正常的 `ToolDefinition` 表示以供分发，但位于可过滤的能力层之外，因此限制规则不会意外移除 PTC mode 的唯一入口。调用遍历完整的工具流水线——`tools/pre-execute` → 单调性守卫 → `tools/execute` 包裹分发 → `tools/post-execute` → 由定义拥有的可选 `finalizeContent` → 不可变的 `tools/result` 通知——与原生调用完全一致；权限插件可以在程序运行前检查程序文本，最终结果观察者看到的是规范化的外层结果。其 `execute(args, exec)`：
...
传输自身的 `description` 与两种 flavor 的 SDK 说明都指出 `code` 必填、`description` 可选，省略时使用 `Run code` 回退标签。调用摘要仍然是模型可见指导，但省略这个仅用于 UI 的字段不会再因 `INVALID_ARGS` 丢弃已写好的程序。
