...
The session row menu in the sidebar workspace browser needed a non-destructive removal action: archive keeps the Session log and workspace accounting untouched while hiding it from every grouping surface (workspace groups, Ungrouped, search, the flat list). Permanent deletion is a separately named and confirmed action. The archive record needs a home: an Ungrouped session belongs to no workspace entity, so a per-workspace field cannot carry it.
...
- UI: `archive` remains a non-danger row with no confirmation dialog — a non-destructive action whose worst misfire is list hiding — alongside the separately confirmed permanent Delete action; filtering is one extra arm in `tree.ts`'s `sessionVisible` predicate, with `deriveGroups`/`deriveFlat` taking an `archived` set parameter so all four surfaces (group loop, stray bucket, search, flat) share one source.
