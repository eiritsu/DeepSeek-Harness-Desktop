...
The Web `minimal` preset and standalone JSON-RPC minimal composition expose persistent `bash` and `str_replace_editor`, but their supporting services do not match the intended training runtime. The Web host supplies context compaction, while the Web preset inherits the host's sandboxed filesystem and the JSON-RPC composition mounts `fs-sandbox` plus filesystem policy. A long Web session can therefore replace history, and the editor advertises and enforces a filesystem policy that the bare local reference runtime does not have.
...
The Web replay boots the complete Web host, creates the agent through the preset service, and asserts that the scoped filesystem is bare, the host compaction service is available, no system-prompt-owned runtime-context message was appended, and the assembled request contains exactly the fixed prompt and two tools. It then executes persistent Bash and the editor against the real scoped services.
...
Standalone minimal sessions never summarize or replace earlier history and never add a runtime-context snapshot; callers must keep turns within the selected model's context capacity and must not rely on model-visible narration of standing sandbox or approval policy. Web minimal sessions use the host compaction policy and share the same no-context guarantee. The editor can address any absolute path visible to the runtime process, independently of the persistent shell's sandbox policy. The two launch paths share their model-facing tool while retaining different prompt, model, and compaction configuration appropriate to their owners. The Python SDK path communicates only through the bundled `dsh` stdio JSON-RPC profile.
