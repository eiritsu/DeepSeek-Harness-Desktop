...
- Session-row … menu: Rename / Fork session / Copy session ID / Archive session / Delete session. Copy, archive, and delete also remain available for a blank row; rename and fork require a materialized prompt. The workspace-header … menu's Rename / Delete workspace actions are both wired. Menus close when the pointer leaves them.
...
- The row action surface is complete; growing the wire status enum remains a future iteration.
