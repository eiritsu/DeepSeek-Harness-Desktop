...
Sidebar workspace 浏览区的会话行菜单需要一项非破坏性移除操作：归档不改动会话日志与 workspace 记账，只把该会话从所有分组视图（workspace 分组、Ungrouped、搜索、平铺列表）里隐藏。永久删除是另行命名并确认的操作。归档记录需要一个落点：Ungrouped 的会话不属于任何 workspace 实体，per-workspace 字段放不下它。
...
- UI：`archive` 保持非 danger 样式且不出确认对话框——这是非破坏性操作，误触后果只是列表隐藏——并与另行确认的永久 Delete 操作并列；过滤实现为 `tree.ts` 的 `sessionVisible` 判据加一档，`deriveGroups`/`deriveFlat` 增加 `archived` 集合入参，四个视图（分组循环、stray 兜底、搜索、平铺）同源生效。
