...
1. **`run_code` gains an optional `description` parameter** (bash's display contract: active voice, 5-10 words, shown in the UI; whitespace-only rejected when explicitly supplied). `presentCall` titles the card with the description or the fixed `Run code` fallback and moves the program to `rawInput`. The prompt-side cost is a few tokens per call; the return is that every surface — TUI card, ACP title, web row — gets a human-readable label without parsing TypeScript.
