...
Pasted or dropped raster images are the Web composer's first consumer of a durable attachment capability. Unsent files remain temporary client-owned draft state. Every rich-content intake adapter decodes its wire blocks, resolves route modality, and delegates the complete image batch to the attachment service before appending its message event. An image-capable route receives the normalized image directly; a text-only route asks the installed attachment recognizer for bounded OCR text and refuses a new image when recognition returns no text. A provider adapter that produces structured image output must durably commit the output before appending its assistant block. Canonical user and assistant content uses role-neutral `ImageBlock` values.

The image path supports PNG, JPEG, WebP, and GIF paste and drag-and-drop, image-only or mixed prompts, historical user and assistant image rendering, and original-image preview on a single click (display and interaction specifics superseded in part by the [attachment-display alignment note](2026-08-11-web-attachment-display-alignment.md)). File and folder intake plus generic durable files are defined by [Generic file attachments with sideloadable recognition](2026-08-23-generic-file-attachments-and-recognition.md); image copying and a custom context menu remain separate follow-ups.
...
`ImageBlock` joins the merge-extensible core `ContentBlockMap` and is valid in either user or assistant content. It never carries base64, an object URL, a filesystem path, or a provider-owned locator. Its optional bounded `recognizedText` records OCR performed for text-only admission, so the session event plus immutable object store reconstructs either the exact model-visible image or its text-only projection without rerunning a mutable recognizer. The LLM vocabulary therefore has a type-only dependency on the attachment seam; provider runtime dependencies remain adapter-specific.
...
- Durable draft staging, image copying, custom context menus, output-provider certification, large-media streaming and native binary model routes, and reference-aware garbage collection remain independent designs.
