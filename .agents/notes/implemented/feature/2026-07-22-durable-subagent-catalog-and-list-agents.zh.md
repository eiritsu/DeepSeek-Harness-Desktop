...
永久删除 Session 不需要 catalog 专用变更。持久化列表会自然移除已删除的 child；任何未来的派生索引都必须移除或 tombstone 同一条目，避免 `list_agents` 保留陈旧状态。
...
- 永久删除的 child 会在持久化列表移除其 Session 后离开 catalog；其他持久化 child 会继续出现在列表里，但存活 Agent 资源仍由驻留 Activation 数量限制。
