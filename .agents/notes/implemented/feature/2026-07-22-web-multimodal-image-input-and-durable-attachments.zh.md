...
粘贴或拖放的光栅图片是 Web 输入区对持久附件能力的首个应用场景。未发送文件仍是由客户端持有的临时草稿状态。每个丰富内容接入适配器都会解码自身协议块、解析路由模态，并在追加消息事件前把完整图片批次委托给附件服务。支持图片的路由直接接收规范化图片；纯文本路由向已安装附件识别器请求有界 OCR 文本，识别没有返回文本时拒绝新图片。生成结构化图片输出的提供方适配器在追加相应助手块前，也必须持久提交输出。规范用户内容与助手内容使用角色无关的 `ImageBlock` 值。

图片路径支持粘贴和拖放 PNG、JPEG、WebP 与 GIF，支持仅图片或混合提示词，支持渲染历史用户图片与助手图片，并支持单击预览原图（展示与交互细节部分由[附件展示对齐 Note](2026-08-11-web-attachment-display-alignment.zh.md)取代）。文件与文件夹准入和通用持久文件由[通用文件附件与可侧载识别](2026-08-23-generic-file-attachments-and-recognition.zh.md)定义；图片复制和自定义上下文菜单仍分别作为后续工作。
...
`ImageBlock` 加入可合并扩展的核心 `ContentBlockMap`，在用户内容和助手内容中都有效。它绝不携带 base64、对象 URL、文件系统路径或提供方持有的定位符。可选有界 `recognizedText` 记录纯文本准入时执行的 OCR，因此会话事件与不可变对象存储可以重建模型可见的确切图片或纯文本投影，而无需重新运行可变识别器。LLM 词汇由此仅在类型层面依赖附件 seam；提供方运行时依赖仍由各适配器持有。
...
- 持久草稿暂存、图片复制、自定义上下文菜单、输出提供方认证、大媒体流式传输与原生二进制模型路由，以及按引用感知的垃圾回收仍是相互独立的设计。
