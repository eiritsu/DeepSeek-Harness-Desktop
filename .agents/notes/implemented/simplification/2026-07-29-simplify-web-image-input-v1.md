...
Exact-model metadata carries only the input modalities that current admission decisions consume. `ImageBlock` carries the durable attachment reference and optional bounded OCR text required by text-only admission; the reference's optional display name supplies accessible UI text, so the core block has no separate user-authored alternative-text field. Provider-neutral token estimation does not apply one provider's visual pricing formula to other routes.
