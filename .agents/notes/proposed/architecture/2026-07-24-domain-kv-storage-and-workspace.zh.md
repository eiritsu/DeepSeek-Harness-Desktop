...
另外，Session 删除需要 `SessionPersistence` 删除原语和 `session.delete` 端点。后续的[永久删除 Session 决策](../../implemented/feature/2026-08-23-permanent-session-deletion.zh.md)已经实现这部分设计；本提案仍为 storage-domain 与介质迁移方向保持活跃。

后续的 [Workspace 注册记录删除决策](../../implemented/feature/2026-07-27-workspace-registration-deletion.zh.md)取代的仅是上述耦合关系：删除 Workspace 注册记录会保留相关 Session 及其日志，永久删除 Session 是独立的显式操作。因此，下文的级联设计并不是 Workspace GUI 的删除语义。
...
| `SessionPersistence.delete` 扩面 + 级联删编排 | `packages/session/*` | 既有 seam 新方法 | ✓ 已作为后续决策独立实现 |
...
依赖方向：`dsh-workspace` → `dsh-domain` → `dsh-storage` ← 两后端。`dsh-workspace` 另依赖 `ctx.sessionPersistence`：attach 读取 header 校验 cwd，删除事件则移除陈旧记账。Host 持有后续删除决策所述的 `ctx.sessions` 运行中检查与级联编排。
...
- **Session 删除是独立能力。** 后续的 [Workspace 注册记录删除决策](../../implemented/feature/2026-07-27-workspace-registration-deletion.zh.md)已将 `ctx.workspaceRegistry.delete(id)` 作为仅删除元数据、保留 Session 与日志的操作交付。[永久删除 Session 决策](../../implemented/feature/2026-08-23-permanent-session-deletion.zh.md)负责递归删除、运行中检查和崩溃重跑收敛。
...
| session delete 约定（已作为后续实现） | 未知 id、已删 id 复用、未物化 intent、与在途 append 串行、deleted 事件 | jsonl、sqlite |
