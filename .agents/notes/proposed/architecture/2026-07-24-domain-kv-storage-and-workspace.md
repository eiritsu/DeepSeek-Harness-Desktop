...
Separately, Session deletion needs a `SessionPersistence` delete primitive and a `session.delete` endpoint. The follow-on [permanent Session deletion decision](../../implemented/feature/2026-08-23-permanent-session-deletion.md) implements that part of this design; this proposal remains active for its storage-domain and medium-migration direction.

The later [Workspace registration deletion decision](../../implemented/feature/2026-07-27-workspace-registration-deletion.md) supersedes only that coupling: deleting a Workspace registration preserves its Sessions and their logs, while permanent Session deletion is a separate explicit action. The cascade design below is therefore not the Workspace GUI delete semantic.
...
| `SessionPersistence.delete` extension + cascade orchestration | `packages/session/session-persistence*` | new method on the existing seam | ✓ follow-on implemented separately |
...
Dependency direction: `dsh-workspace` → `dsh-domain` → `dsh-storage` ← the two backends. `dsh-workspace` additionally depends on `ctx.sessionPersistence`: attach reads headers for cwd verification, while the deletion event removes stale accounting. The Host owns the `ctx.sessions` running check and cascade orchestration described by the follow-on deletion decision.
...
- **Session deletion is a separate capability.** The later [Workspace registration deletion decision](../../implemented/feature/2026-07-27-workspace-registration-deletion.md) ships `ctx.workspaceRegistry.delete(id)` as a metadata-only operation that preserves Sessions and logs. The [permanent Session deletion decision](../../implemented/feature/2026-08-23-permanent-session-deletion.md) owns recursive deletion, running checks, and crash-rerun convergence.
...
| session delete contract (implemented follow-on) | unknown id, deleted-id reuse, un-materialized intent, serialization with in-flight appends, the deleted event | jsonl, sqlite |
