...
|---|---|
| Complete model metadata and reasoning capability | `dsh-model-catalog` refreshes upstream `models.dev` declarations and supplements `llm-pi-ai` routes without creating a provider; conservative same-ID merging prevents an opaque gateway from advertising capacity or modalities that one upstream declaration does not support |
...
The SQLite migration test constructs the original desktop schema and a released v0 Session, then verifies startup rewrites it through the installed v0-to-v3 catalog before `stat`, `open`, or append can observe it. The adjacent migration admits released Lark message provenance and converts its historical file-block `recognizedText` representation into the current DeepSeek Files text block without discarding the attachment or transport ids. A real `desktop-lite` configuration dump pins the provider swap as two Loader operations: disabling the base JSONL row and inserting the SQLite provider under its own id. This prevents the include patcher's package-name guard from skipping the swap. The JSONL and SQLite persistence suites, including durable deletion, currently pass 210 tests. Swift's 47 tests include authoritative Session database export, import, reset, and schema validation.
