...
- **An image is refused before sending** — Fetch the provider's available models again. The Models page automatically preserves explicit input modalities returned by the installed catalog or the endpoint; if the endpoint discloses none, declare `input: [text, image]` for that custom model. DeepSeek's own chat-completions route is text-only and cannot be configured otherwise.
