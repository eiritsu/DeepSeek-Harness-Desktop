...
The block interfaces (full fields in source): `TextBlock` (`text`), `ReasoningBlock` (thinking, distinct from visible text), `ImageBlock` (a durable [image attachment](attachment.md) plus optional OCR text), `FileBlock` (a durable generic file attachment plus optional recognized text), `ToolCallBlock` (`id: CallId`, `name`, raw-JSON `arguments`), and `ToolResultBlock` (`toolCallId`, nested `content: ContentBlock[]`, `isError?`). `ContentBlock = ContentBlockMap[ContentBlockType]`. A new modality belongs in the merge-extensible map only when its adapter, UI, compaction, and durable replay paths honor it.
 ...
 * a surface adopting one still owes any required metadata the provider omits.
 */
...
Discovery results pass through ordered `LlmModelDiscoveryEnricher` registrations after provider normalization. Each registration sees detached candidates after earlier enrichers and may return patches only for ids already present; provider-disclosed fields and earlier patches win. This is a metadata-composition point, not a routing or capability inference mechanism.

```ts type-equiv
/** Input presented to one model-discovery enrichment plugin. */
interface LlmModelDiscoveryEnrichmentRequest {
  /** Settings namespace whose discovery produced the candidates. */
  settingsNs: string
  /** Original provider draft used for discovery. */
  request: LlmModelDiscoveryRequest
  /** Detached candidates after earlier enrichers have filled missing metadata. */
  models: readonly LlmDiscoveredModel[]
}
```

```ts type-equiv
/** One plugin that supplies missing metadata for already-discovered model ids. */
interface LlmModelDiscoveryEnricher {
  /**
   * Return candidate patches keyed by id; fields already disclosed by the
   * provider or an earlier enricher remain authoritative.
   * @param input - provider draft and detached candidates to inspect.
   * @returns metadata patches for ids already present in `input.models`.
   */
  (input: LlmModelDiscoveryEnrichmentRequest): Promise<readonly LlmDiscoveredModel[]>
}
```

Exact-model input resolvers are a runtime fallback for adapters that have neither a per-model declaration nor native catalog metadata. The first registered resolver with an answer wins; adapters apply it to both reported capability and provider dispatch without mutating settings.

```ts type-equiv
/** Exact route/model identity presented to an input-modality resolver. */
interface LlmModelInputRequest {
...
```ts type-equiv
/** One plugin that supplies exact model input modalities when an adapter has no answer. */
interface LlmModelInputResolver {
  /**
   * Resolve exact input modalities, or return `undefined` to delegate to the
   * next resolver and ultimately the adapter's own fallback.
...
/**
 * Register one ordered plugin that fills metadata missing from model
 * discovery results. Provider-disclosed values and earlier enrichers win;
 * patches for ids absent from the discovery result are ignored.
 * @param enrich - receives detached candidates and returns metadata patches.
 * @returns disposer that withdraws only this enrichment registration.
 */
registerModelDiscoveryEnricher(enrich: LlmModelDiscoveryEnricher): () => void

/**
 * Register one ordered exact-model input resolver. Adapters consult this
 * according to their own explicit-configuration precedence; the first
 * resolver with an answer wins.
...
/**
 * Resolve exact input modalities from registered external catalogs.
