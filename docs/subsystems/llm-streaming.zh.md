...
提供方归一化之后，发现结果会按注册顺序通过 `LlmModelDiscoveryEnricher`。每个注册项看到的是更早 enricher 处理后的分离候选，并且只能返回已有 id 的 patch；提供方明确公布的字段和更早 patch 优先。这里是元数据组合点，不是路由或能力推断机制。
...
精确模型输入解析器是运行时回退，仅供既没有逐模型声明、也没有原生 catalog 元数据的适配器使用。首个给出答案的注册解析器胜出；适配器把结果同时用于能力报告和提供方分发，并且不修改 settings。
