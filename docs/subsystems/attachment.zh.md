...
附件 seam 将二进制所有权与会话日志分离。生产方把编码字节交给 [`ctx.attachments`](#ctxattachments--attachmentstore-abstract-seam)；只有对象完成持久化后，该服务才会发布不可变的内容寻址引用。会话事件包含文件或图片引用及其元数据，绝不包含浏览器对象 URL、宿主临时路径、提供方 URL 或 base64 数据。

未发送的浏览器草稿可以保留在内存中，原生客户端也可以将其暂存于操作系统临时存储。宿主接受用户消息后，会先把消息中的附件移到 `<DSH_HOME>/attachments/v1` 下，再追加用户事件。结构化模型图片输出遵循同样的先持久化、后追加事件规则。
...
`saveFiles()` 执行通用文件数量与字节限制，逐字节保留文件，并发布有序引用。`admitEncodedFiles()` 把空浏览器媒体类型规范化为 `application/octet-stream`，并拒绝非规范 base64。`readFile()` 校验摘要、字节长度和已记录元数据。第一个支持文件或规范化图片的已注册 `FileRecognizer` 可以在 prompt 准入时提取有界文本。宿主把文件文本记录在 `FileBlock`，把图片 OCR 文本记录在 `ImageBlock`；请求投影会输出稳定文件说明并追加识别文本，或在所选模型只接受文本时用持久 OCR 文本替代图片。通用文件识别失败不会阻止存储或虚构内容。文本模型接收的新图片在没有识别器返回文本时会被拒绝。
