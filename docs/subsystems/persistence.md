...
/**
 * Permanently delete one session's stored log. Operations for the same id
 * remain serialized with appends and recovery. A live Session rejects;
 * callers must first stop and dispose its owning lifecycle. An unknown id
 * rejects, while a lazy create intent with no artifact is cancelled.
...
Emitted after one session identity has left durable storage. Consumers use this edge to remove derived records; the deleted log cannot be read again when listeners run.

```ts cordis-catalog
/**
 * Emitted after one session identity has left durable storage. Consumers
 * use this edge to remove derived records; the deleted log cannot be read
 * again when listeners run.
