...
<p align="center"><img src="desktop-shell/Resources/AppIcon.svg" width="112" alt="DeepSeek Harness Desktop 图标"></p>

![DeepSeek Harness Desktop 主界面](desktop-shell/docs/images/home.png)
...
默认 macOS 应用无需额外安装即可使用以下组件：

- **插件库。** `@deepseek-ai/dsh-client-ui-plugin-library` 桌面界面提供已安装插件清单、URL 审查、社区发现和操作日志。它属于应用组件，不是可移除的 profile Bundle。
- **Deepseek-Files。** 默认 [`@deepseek-ai/dsh-file-recognizer-office`](packages/attachment/file-recognizer-office/README.zh.md) Bundle 可接收文件与文件夹，在本地提取支持的文本、Markdown、表格、Office、OpenDocument 与 PDF 内容；当前模型无法原生接收时，还可调用已配置的 OCR、音频转写或视频理解兜底。
- **模型能力目录。** 默认 [`@deepseek-ai/dsh-model-catalog`](packages/llm/model-catalog/README.zh.md) Bundle 会从 `models.dev` 刷新精确输入模态声明，保留 last-good 快照，并回退 pi-ai 内置 catalog；它绝不根据模型名称或协议猜测能力。

具有随发行版提供的应用前缀的既有 Web profile，会在下次启动时补齐缺失的内置 Bundle，同时保留第三方 Bundle 条目。
...
![复制 Session ID、归档和删除会话](desktop-shell/docs/images/session-actions.png)

会话菜单提供三项桌面版补充操作。重新启动应用时会恢复此前选中的会话，不会每次都创建并选中一个空白新会话。

### 自定义模型供应商

![已隐藏私有供应商名称的模型供应商设置](desktop-shell/docs/images/model-providers-redacted.png)

这张文档截图已经隐藏自定义供应商名称。公开供应商和公开模型名称可以保留，但私有供应商标签、Base URL 与 API Key 不应提交到文档。

### 社区发现

![带分类、搜索和排序的社区插件发现](desktop-shell/docs/images/plugin-discovery.png)
...
![固定桌面画布中的插件 URL 审查](desktop-shell/docs/images/plugin-review.png)
...
![插件审查与安装操作日志](desktop-shell/docs/images/plugin-audit-log.png)
